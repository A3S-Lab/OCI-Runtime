...
nine start, all nine kill, and all nine delete stages through
`HostRuntimeService` reopen. Faults before guest dispatch leave the durable
operation resumable and perform the first effect on the replacement connection.
Faults after dispatch replay the cached guest response, including a guest that
...
generation while the durable host still records `stopped`. A fault after the
...
driver, preserves the same generation, and produces one effect per operation;
