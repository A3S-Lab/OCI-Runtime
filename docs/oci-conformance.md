...
| Linux cgroup resources | Yes | Initial CPU, block I/O, and RDMA relationships | Bootstrap slice creates an explicit cgroup v2 path, enables required controllers, applies and reads back memory limit/reservation/swap, CPU quota/period/shares/cpuset, and PID limits, joins init plus exec, and verifies pause/resume through `cgroup.freeze` and `cgroup.events`; I/O, hugetlb, RDMA, unified resources, delegation breadth, and complete lifecycle evidence remain incomplete | No |
...
| `create/state/start/kill/delete` plus init `wait` | SDK contract | Exhaustive request boundary, protocol-v1/v2 compatibility tests, protocol-v3 process-message tests, protocol-v4 control-message tests, durable lifecycle tests, and native/utility-VM lifecycle gates | Driver-independent core orchestration plus driver-advertised stable init wait and a fail-closed guest bootstrap slice | No |
| Hooks and rollback ordering | SDK contract | Pending | No | No |
| Exec, I/O, PTY, per-process wait, pause/resume, processes, update | SDK contract | Typed requests; protocol-v3 exact process target/correlation tests; protocol-v4 exact freezer and inventory tests; exhaustive durable process/freezer recovery and driver-boundary matrices; real frozen/resumed workload-progress evidence | Native Linux exposes durable exact-target exec, pidfd-backed process signal, stable process wait, init-exit supervision, pause/resume, live process inventory, and cleanup for null I/O through `RuntimeClient`; the shared utility-VM guest executor exposes the same controls, while stream I/O, PTY, update, and the utility-VM host adapter remain unenforced | No |
