...
| POSIX hooks | Yes | Absolute path, environment shape, positive timeout, and executor bounds | Shared Linux executor runs all six phases in normative order and namespace, passes exact bounded OCI state on stdin, enforces timeout cleanup for the complete private process group including signal-resistant background descendants, rolls back independent prestart, createRuntime, createContainer, startContainer, and poststart failures, and continues warning-only poststop hooks | No |
...
| Checkpoint and restore | SDK contract | Protocol-v8 typed single-file paths, exact paused source generations, immutable content/configuration/attachment/driver/Host-build/driver-build/format references, request-bound paused responses, legacy-semantics rejection, and capability-advertisement exclusion tests | Host and production drivers remain fail-closed pending durable atomic creation, scoped partial cleanup, read-only restore verification, replay, and real-host qualification | No |
...
8. a versioned ten-case real-Guest path-isolation profile, wired into macOS
...
3. hook crash-recovery, security-negative, and adversarial hook-soak traces
...
6. upstream OCI lifecycle validation and independent cross-runtime results for
