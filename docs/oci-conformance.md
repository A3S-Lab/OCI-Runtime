...
| Linux Intel RDT, memory policy, time offsets, net devices | Yes | Memory-policy mode presence, bounded node lists, and mode/flag relationships; initial Intel RDT, time-offset, and network-device rules | Bootstrap slice applies all seven recognized NUMA memory-policy modes and three flags to configured init with immediate kernel read-back, preserves inherited policy on omission, reports the same registry through OCI Features, and applies and reads back normalized monotonic/boottime offsets; Intel RDT and network devices remain unenforced | No |
...
3. promotion of all 381 pending common, Linux, and VM normative entries to
