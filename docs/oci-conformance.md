...
| File transfer and filesystem sessions | SDK contract | Protocol-v4 SDK transport and protocol-v9 agent round trips; exact request-shape, size/depth, capability, target-correlation, generation-fence, and driver-boundary tests | The shared executor confines paths to a retained rootfs descriptor with `openat2` and descriptor-relative syscalls, maps container users through the retained namespace, bounds payloads and listings, and journals upload/mkdir/move/remove by `OperationId`; real-host qualification remains pending | No |
