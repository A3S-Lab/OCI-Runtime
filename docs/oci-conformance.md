...
| File transfer and filesystem sessions | SDK contract | Protocol-v4 SDK transport, protocol-v9 file/filesystem round trips, and protocol-v10 Guest replay-record acknowledgement; exact request-shape, size/depth, capability, target-correlation, generation-fence, and driver-boundary tests | The shared executor confines paths to a retained rootfs descriptor with `openat2` and descriptor-relative syscalls, maps container users through the retained namespace, bounds payloads and listings, and journals upload/mkdir/move/remove by `OperationId`; Host File/Filesystem mutations still need durable journaling before their Guest records can use the post-commit acknowledgement boundary | No |
