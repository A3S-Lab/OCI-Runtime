...
Runtime Tools suites, and non-Linux platform package gates remain outside the
qualified x86_64 and AArch64 core profiles.
...
5. exact-package x86_64 and AArch64 OCI command-line lifecycle qualifications
   using the same pinned Runtime Tools source and compatibility-locked Alpine
   3.22.5 minirootfs archives. Each archive is bound by URL, size, SHA-256,
   path safety, required BusyBox identities, and ELF architecture before it is
   published under the `rootfs-${GOARCH}.tar.gz` name expected upstream. All
   nine selected tests execute on each architecture; seven pass raw TAP, while
   `start` and `pidfile` retain two exact source-audited upstream harness-defect
   signatures plus the runtime's conformant state, error, cleanup,
   retired-journal, and clean-shutdown evidence;
