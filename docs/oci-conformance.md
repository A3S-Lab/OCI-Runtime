...
| Common root, mounts, process, hostname, annotations | Yes | Initial cross-field rules; three `oomScoreAdj` entries are enforced while the remaining normative manifest is pending | Bootstrap slice applies ordered mounts with missing target creation, four rootfs propagation modes, recursive VFS attributes, detached ID-mapped filesystem and bind mounts, masked/read-only paths, read-only rootfs, hostname/domainname, credentials, all 16 OCI process rlimit types, exact optional `oomScoreAdj`, and process launch for init and exec; native Linux reads back distinct configured `RLIMIT_NOFILE` values and exact configured `oom_score_adj` values in init and exec, and retains real shared read-write/read-only bind, private-tmpfs, inline-script, executable-script, direct-argv, and exact nonzero-exit evidence on x86_64 and aarch64 | No |
...
3. promotion of all 627 pending common, Linux, and VM normative entries to
