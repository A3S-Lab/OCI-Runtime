...
| Linux cgroup resources | Yes | Initial CPU, block I/O, and RDMA relationships | Bootstrap slice creates an explicit cgroup v2 path, enables required controllers, applies and reads back memory limit/reservation/swap, CPU quota/period/shares/cpuset, and PID limits, and joins init plus exec; I/O, hugetlb, RDMA, unified resources, delegation breadth, and lifecycle evidence remain incomplete | No |
...
| Exec, I/O, PTY, per-process wait, pause/resume, update | SDK contract | Typed requests; protocol-v3 exact target/correlation tests; exhaustive durable process recovery and driver-boundary matrices; fail-closed shared process planning | Native Linux exposes durable exact-target exec, pidfd-backed process signal, stable process wait, init-exit supervision, and cleanup for null I/O through `RuntimeClient`; stream I/O, PTY, pause/resume, update, and utility-VM host adapters remain unenforced | No |
