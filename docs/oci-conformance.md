...
| `create/state/start/kill/delete` plus init `wait` | SDK contract | Exhaustive request boundary, protocol-v1/v2 compatibility tests, protocol-v3 process-message tests, protocol-v4 control-message tests, protocol-v5 resource-message tests, durable lifecycle tests, and native/utility-VM lifecycle gates | Driver-independent core orchestration plus driver-advertised stable init wait and a fail-closed guest bootstrap slice | No |
| Hooks and rollback ordering | SDK contract | Pending | No | No |
| Exec, I/O, PTY, per-process wait, pause/resume, processes, update, stats | SDK contract | Typed requests; protocol-v3 exact process target/correlation tests; protocol-v4 exact freezer and inventory tests; protocol-v5 live-update and typed-stats tests; exhaustive durable process/freezer/update recovery and driver-boundary matrices; real update/stats plus frozen/resumed workload-progress evidence | Native Linux exposes durable exact-target exec, pidfd-backed process signal, stable process wait, init-exit supervision, pause/resume, live process inventory, partial live cgroup resource updates, normalized stats, and cleanup for null I/O through `RuntimeClient`; the shared utility-VM guest executor exposes the same controls, while stream I/O, PTY, and the utility-VM host adapter remain unenforced | No |
