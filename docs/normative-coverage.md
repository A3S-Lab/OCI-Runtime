...
| `enforced` | 25 | Root `config.json` placement; post-create configuration immutability; start, kill, and delete state gates; start-process presence; and OCI `oomScoreAdj`, scheduler, and I/O-priority semantics enforced by the bundle loader, runtime lifecycle, and Linux executor |
| `pending-review` | 604 | Common, Linux, or VM entries awaiting exact evidence binding |
