...
| `pending-review` | 148 | Common, Linux, or VM entries awaiting exact evidence binding |
...
ID-mapped mounts are bound to exact report and executor tests. Of the feature
documents' previously pending entries, only the recommendation to determine
the entire report at compile time remains open; A3S composes driver-specific
capabilities from the configured runtime service.
...
The Linux-device review promotes 19 entries. Schema and semantic checks cover
...
device-access rules preserve wildcard and reset semantics; omitted or empty
access masks are no-ops and an all-no-op list performs no BPF load. One
separate requirement remains pending: proving that `CAP_MKNOD` and every
late-mounted source cannot expose an undeclared device node.
