...
| `enforced` | 41 | Root `config.json` placement; required create and Query State arguments; valid and unique create IDs; exact Query State results; post-create configuration immutability; start, kill, and delete state gates; start-process presence; required OCI State fields, Linux PID lifecycle, annotations, and schema; and OCI `oomScoreAdj`, scheduler, and I/O-priority semantics enforced by the SDK transport, bundle loader, runtime lifecycle, and Linux executor |
| `pending-review` | 588 | Common, Linux, or VM entries awaiting exact evidence binding |
