...
| `enforced` | 34 | Root `config.json` placement; post-create configuration immutability; start, kill, and delete state gates; start-process presence; required OCI State fields, host-unique IDs, Linux PID lifecycle, annotations, and schema; and OCI `oomScoreAdj`, scheduler, and I/O-priority semantics enforced by the bundle loader, runtime lifecycle, and Linux executor |
| `pending-review` | 595 | Common, Linux, or VM entries awaiting exact evidence binding |
