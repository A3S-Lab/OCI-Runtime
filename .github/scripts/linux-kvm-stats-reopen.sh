...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM Stats reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM Stats reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM Stats reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-stats-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM Stats reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-stats-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM Stats reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM Stats reopen stage %s failed with status %s near line %s\n' \
    "$current_stage" "$status" "${BASH_LINENO[0]}" >&2
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        durable_running_retained, update_journal_succeeded_before_reopen,
        first_operation_response_received, first_stats_verified,
        first_operation_dispatches,
        ...
        replacement_stats_verified, stats_snapshot_rebound,
...
report_path="${A3S_OCI_LINUX_KVM_STATS_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM Stats reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-stats-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-stats-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-stats-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-stats-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-stats-reopen
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM Stats reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-stats-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "stats"
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-stats-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-stats-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-stats-reopen-"))
     and (.setup_update_operation_id | startswith("kvm-stats-reopen-"))
     and ([.qualification_operation_id, .setup_create_operation_id,
           .setup_start_operation_id, .setup_update_operation_id]
          | unique | length) == 4
     ...
     and (.container_id | startswith("kvm-stats-reopen-"))
     ...
     and .injected_point == ("agent-v10.stats-" + $stage)
     ...
     and .replacement_stats_verified and .stats_snapshot_rebound
     ...
     and (if $stage == "guest-after-response-write" then
            .replacement_stats_snapshot.timestamp_unix_ns
