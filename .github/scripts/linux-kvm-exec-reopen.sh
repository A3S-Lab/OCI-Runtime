...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM Exec reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM Exec reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM Exec reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-exec-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM Exec reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-exec-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM Exec reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM Exec reopen stage %s failed with status %s near line %s\n' \
    "$current_stage" "$status" "${BASH_LINENO[0]}" >&2
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        durable_running_retained, exec_journal_prepared_before_reopen,
        exec_journal_succeeded_before_reopen, first_exec_pid,
        ...
        replacement_operation_dispatches, replacement_exec_pid,
        exec_response_rebound, host_changed_request_rejected,
        host_stale_generation_rejected, guest_stale_generation_rejected,
        replacement_workload_verified, replacement_exec_marker_verified,
...
report_path="${A3S_OCI_LINUX_KVM_EXEC_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM Exec reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-exec-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-exec-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-exec-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-exec-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-exec-reopen
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM Exec reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-exec-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "exec"
     and .exec_terminal
     and (.exec_process_id | startswith("worker-"))
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-exec-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-exec-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-exec-reopen-"))
     ...
     and (.container_id | startswith("kvm-exec-reopen-"))
     and .negotiated_protocol == 10
     and .injected_point == ("agent-v10.exec-" + $stage)
     ...
     and (if $stage == "guest-after-response-write" then
            ...
            and .exec_journal_prepared_before_reopen
