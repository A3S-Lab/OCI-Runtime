...
run_close_stdin_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/close-stdin/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "close-stdin"
       ...
       and .first_close_marker_verified
