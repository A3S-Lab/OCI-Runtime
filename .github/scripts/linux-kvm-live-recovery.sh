...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM Live recovery qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM Live recovery command is unavailable: %s\n' "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM Live recovery architecture: %s\n' "$architecture" >&2
    exit 2
    ;;
esac
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM Live recovery work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-klr.*)
      ...
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM Live recovery path: %s\n' "$work" >&2
      return 1
      ;;
  esac
}
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM Live recovery qualification failed with status %s near line %s\n' \
...
report_path="${A3S_OCI_LINUX_KVM_LIVE_RECOVERY_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM Live recovery report: %s\n' "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-live-host-reopen-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-live-recovery-matrix.v1",
      platform: "linux",
...
jq --exit-status \
  ...
  '.schema_version == "a3s.oci.linux-kvm-live-recovery-smoke.v1"
   ...
   and .recovery.session_owner_mode_durable
   ...
   and (.recovery.live_vm_processes_reaped | not)
   ...
   and .recovery.process_inventory_nonempty
