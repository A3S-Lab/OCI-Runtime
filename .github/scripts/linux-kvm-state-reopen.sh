...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM State reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM State reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM State reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-state-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM State reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-state-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM State reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM State reopen stage %s failed with status %s near line %s\n' \
    "$current_stage" "$status" "${BASH_LINENO[0]}" >&2
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        durable_created_retained, first_operation_response_received,
        host_service_reopened, replacement_recovery_calls,
        replacement_rehydrated_created_record, owners_distinct,
        state_root_removed}' \
...
report_path="${A3S_OCI_LINUX_KVM_STATE_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM State reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-state-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-state-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-state-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-state-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-state-reopen
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM State reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-state-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "state"
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-state-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-state-reopen-"))
     and .qualification_operation_id != .setup_create_operation_id
     and (.container_id | startswith("kvm-state-reopen-"))
     ...
     and .durable_created_retained and (.first_created_pid > 0)
