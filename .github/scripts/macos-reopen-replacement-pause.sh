run_pause_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/pause/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "pause"
       and .requested_stage == $stage
       and (.qualification_operation_id | startswith("pause-reopen-"))
       and (.setup_create_operation_id | startswith("pause-reopen-"))
       and (.setup_start_operation_id | startswith("pause-reopen-"))
       ...
       and (.container_id | startswith("smoke-pause-reopen-"))
