run_signal_process_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/signal-process/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "signal-process"
       ...
       and (.qualification_operation_id
            | startswith("signal-process-reopen-"))
       ...
       and (.container_id | startswith("smoke-signal-process-reopen-"))
       ...
       and .first_signal_marker_verified
