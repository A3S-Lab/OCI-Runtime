...
# fresh accepts the full soak profile
...
[[ "$(linux_kvm_compute_promotes_readiness existing '{"schema_version":"a3s.oci.linux-kvm-fresh-host-attestation.v1"}' 0 1 25 25)" == "false" ]]

# fresh + attestation + full soak promotes
[[ "$(linux_kvm_compute_promotes_readiness fresh '{"schema_version":"a3s.oci.linux-kvm-fresh-host-attestation.v1"}' 0 1 25 25)" == "true" ]]

# fresh + attestation but soak skipped does not promote
[[ "$(linux_kvm_compute_promotes_readiness fresh '{"schema_version":"a3s.oci.linux-kvm-fresh-host-attestation.v1"}' 1 0 0 0)" == "false" ]]

# fresh + attestation but soak did not run does not promote
[[ "$(linux_kvm_compute_promotes_readiness fresh '{"schema_version":"a3s.oci.linux-kvm-fresh-host-attestation.v1"}' 0 0 0 0)" == "false" ]]

# fresh + attestation but reduced soak depth does not promote
[[ "$(linux_kvm_compute_promotes_readiness fresh '{"schema_version":"a3s.oci.linux-kvm-fresh-host-attestation.v1"}' 0 1 1 1)" == "false" ]]

# fresh without attestation does not promote
[[ "$(linux_kvm_compute_promotes_readiness fresh 'null' 0 1 25 25)" == "false" ]]
