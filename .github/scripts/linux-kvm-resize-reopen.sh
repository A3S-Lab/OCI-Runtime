...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM Resize reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM Resize reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM Resize reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-resize-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM Resize reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-resize-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM Resize reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM Resize reopen stage %s failed with status %s near line %s\n' \
  ...
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        ...
        resize_journal_prepared_before_reopen,
        resize_journal_succeeded_before_reopen,
        ...
        replacement_rehydrated_resize, replacement_operation_dispatches,
        ...
        replacement_exec_marker_verified, replacement_resize_marker_verified,
...
report_path="${A3S_OCI_LINUX_KVM_RESIZE_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM Resize reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-resize-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-resize-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-resize-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-resize-reopen-9-stage-v1"
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM Resize reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-resize-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "resize"
     ...
     and (.qualification_operation_id | startswith("kvm-resize-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-resize-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-resize-reopen-"))
     and (.setup_exec_operation_id | startswith("kvm-resize-reopen-"))
     ...
     and (.container_id | startswith("kvm-resize-reopen-"))
     and .negotiated_protocol == 10
     and .injected_point == ("agent-v10.resize-" + $stage)
     ...
     and .first_exec_marker_verified and .first_resize_marker_verified
