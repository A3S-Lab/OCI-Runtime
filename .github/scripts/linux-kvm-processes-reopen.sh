...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM Processes reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM Processes reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM Processes reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-processes-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM Processes reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-processes-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM Processes reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM Processes reopen stage %s failed with status %s near line %s\n' \
  ...
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        ...
        first_process_inventory_verified, first_operation_dispatches,
        ...
        replacement_process_inventory_verified, process_inventory_rebound,
...
report_path="${A3S_OCI_LINUX_KVM_PROCESSES_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM Processes reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-processes-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-processes-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-processes-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-processes-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-processes-reopen
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM Processes reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-processes-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '. as $report
     ...
     and .bundle_loaded and .requested_operation == "processes"
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-processes-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-processes-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-processes-reopen-"))
     and (.setup_exec_operation_id | startswith("kvm-processes-reopen-"))
     ...
     and (.container_id | startswith("kvm-processes-reopen-"))
