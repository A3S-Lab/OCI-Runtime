run_exec_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/exec/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "exec"
       and .exec_terminal
       and (.exec_process_id | startswith("worker-"))
       and .requested_stage == $stage
       and (.qualification_operation_id | startswith("exec-reopen-"))
       and (.setup_create_operation_id | startswith("exec-reopen-"))
       and (.setup_start_operation_id | startswith("exec-reopen-"))
       ...
       and (.container_id | startswith("smoke-exec-reopen-"))
