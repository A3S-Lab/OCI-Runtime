...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM SignalProcess reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM SignalProcess reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM SignalProcess reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-signal-process-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM SignalProcess reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-signal-process-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM SignalProcess reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM SignalProcess reopen stage %s failed with status %s near line %s\n' \
  ...
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        ...
        signal_process_journal_prepared_before_reopen,
        signal_process_journal_succeeded_before_reopen,
        ...
        replacement_rehydrated_signal_process, replacement_operation_dispatches,
        ...
        replacement_exec_marker_verified, replacement_signal_marker_verified,
...
report_path="${A3S_OCI_LINUX_KVM_SIGNAL_PROCESS_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM SignalProcess reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-signal-process-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-signal-process-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-signal-process-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-signal-process-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-signal-process-reopen
init_marker="$bundle/rootfs/.a3s-oci-create-start-smoke"
exec_marker="$bundle/rootfs/.a3s-oci-exec-reopen-smoke"
signal_marker="$bundle/rootfs/.a3s-oci-signal-process-reopen-smoke"
test ! -e "$init_marker"
test ! -e "$exec_marker"
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM SignalProcess reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-signal-process-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "signal-process"
     and .signal_process_signal == 10 and .exec_terminal
     and (.exec_process_id | startswith("signal-worker-"))
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-signal-process-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-signal-process-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-signal-process-reopen-"))
     and (.setup_exec_operation_id | startswith("kvm-signal-process-reopen-"))
     ...
     and (.container_id | startswith("kvm-signal-process-reopen-"))
     and .negotiated_protocol == 10
     and .injected_point == ("agent-v10.signal-process-" + $stage)
     ...
     and (if $stage == "guest-after-response-write" then
            ...
            and .signal_process_journal_prepared_before_reopen
     ...
     and .first_exec_marker_verified and .first_signal_marker_verified
     ...
     and .signal_marker_reset_before_replacement
     and .replacement_signal_marker_verified
     ...
     and .signal_marker_absent_after_cleanup
