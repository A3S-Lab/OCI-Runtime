...
run_update_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/update/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "update"
       and .requested_stage == $stage
       and (.qualification_operation_id | startswith("update-reopen-"))
       and (.setup_create_operation_id | startswith("update-reopen-"))
       and (.setup_start_operation_id | startswith("update-reopen-"))
       ...
       and (.container_id | startswith("smoke-update-reopen-"))
       ...
       and (.replacement_update_stats.metrics
