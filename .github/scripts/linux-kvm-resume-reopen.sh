...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM Resume reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM Resume reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM Resume reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-resume-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM Resume reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-resume-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM Resume reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM Resume reopen stage %s failed with status %s near line %s\n' \
    "$current_stage" "$status" "${BASH_LINENO[0]}" >&2
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        durable_running_retained, durable_paused_retained,
        pause_journal_succeeded_before_reopen,
        resume_journal_prepared_before_reopen,
        resume_journal_succeeded_before_reopen,
        ...
        replacement_rehydrated_paused_record,
        replacement_rehydrated_resumed_record,
        replacement_operation_dispatches,
...
report_path="${A3S_OCI_LINUX_KVM_RESUME_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM Resume reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-resume-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-resume-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-resume-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-resume-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-resume-reopen
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM Resume reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-resume-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "resume"
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-resume-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-resume-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-resume-reopen-"))
     and (.setup_pause_operation_id | startswith("kvm-resume-reopen-"))
     and ([.qualification_operation_id, .setup_create_operation_id,
           .setup_start_operation_id, .setup_pause_operation_id]
          | unique | length) == 4
     and (.setup_kill_operation_id == null)
     and (.setup_exec_operation_id == null)
     and (.setup_signal_process_operation_id == null)
     and (.container_id | startswith("kvm-resume-reopen-"))
