...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM WaitProcess reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM WaitProcess reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM WaitProcess reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-wait-process-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM WaitProcess reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-wait-process-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM WaitProcess reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM WaitProcess reopen stage %s failed with status %s near line %s\n' \
  ...
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        ...
        process_exit_cached_before_reopen, process_exit_cached_after_reopen,
        ...
        replacement_exec_pid, exec_response_rebound, replacement_wait_exit_status,
        cached_wait_exit_status, operation_replayed_without_driver_dispatch,
        cached_wait_replayed_without_driver_dispatch, host_stale_generation_rejected,
        guest_stale_generation_rejected, replacement_workload_verified,
        replacement_exec_marker_verified,
...
report_path="${A3S_OCI_LINUX_KVM_WAIT_PROCESS_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM WaitProcess reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-wait-process-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-wait-process-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-wait-process-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-wait-process-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-wait-process-reopen
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM WaitProcess reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-wait-process-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "wait-process"
     ...
     and (.qualification_operation_id | startswith("kvm-wait-process-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-wait-process-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-wait-process-reopen-"))
     and (.setup_exec_operation_id | startswith("kvm-wait-process-reopen-"))
     and (.setup_signal_process_operation_id
          | startswith("kvm-wait-process-reopen-"))
     ...
     and (.container_id | startswith("kvm-wait-process-reopen-"))
     and .negotiated_protocol == 10
     and .injected_point == ("agent-v10.wait-process-" + $stage)
