...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM CloseStdin reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM CloseStdin reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM CloseStdin reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-close-stdin-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM CloseStdin reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-close-stdin-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM CloseStdin reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM CloseStdin reopen stage %s failed with status %s near line %s\n' \
  ...
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        ...
        close_stdin_journal_prepared_before_reopen,
        close_stdin_journal_succeeded_before_reopen,
        ...
        replacement_rehydrated_close_stdin, replacement_operation_dispatches,
        ...
        replacement_exec_marker_verified, replacement_close_marker_verified,
...
report_path="${A3S_OCI_LINUX_KVM_CLOSE_STDIN_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM CloseStdin reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-close-stdin-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-close-stdin-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-close-stdin-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-close-stdin-reopen-9-stage-v1"
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM CloseStdin reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-close-stdin-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "close-stdin"
     and (.signal_process_signal == null)
     and (.exec_terminal | not)
     and (.exec_process_id | startswith("close-worker-"))
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-close-stdin-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-close-stdin-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-close-stdin-reopen-"))
     and (.setup_exec_operation_id | startswith("kvm-close-stdin-reopen-"))
     ...
     and (.container_id | startswith("kvm-close-stdin-reopen-"))
     ...
     and (if $stage == "guest-after-response-write" then
            ...
            and (.close_stdin_journal_prepared_before_reopen | not)
            and .close_stdin_journal_succeeded_before_reopen
            and .replacement_rehydrated_close_stdin
            and .operation_replayed_without_driver_dispatch
          else
            (.first_response_matches_durable_record | not)
            and .close_stdin_journal_prepared_before_reopen
            and (.close_stdin_journal_succeeded_before_reopen | not)
            and (.replacement_rehydrated_close_stdin | not)
     ...
     and .first_exec_marker_verified and .first_close_marker_verified
     ...
     and .close_stdin_request_identity_reused
     ...
     and .close_marker_reset_before_replacement
     and .replacement_close_marker_verified
     ...
     and .close_marker_absent_after_cleanup
