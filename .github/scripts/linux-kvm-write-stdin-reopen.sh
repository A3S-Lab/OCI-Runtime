...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf 'Linux KVM WriteStdin reopen qualification requires a Linux host\n' >&2
  exit 2
fi
...
do
  if ! command -v "$command" >/dev/null 2>&1; then
    printf 'required Linux KVM WriteStdin reopen command is unavailable: %s\n' \
      "$command" >&2
    exit 1
  fi
done
...
case "$architecture" in
  ...
  aarch64)
    ...
    ;;
  *)
    printf 'unsupported Linux KVM WriteStdin reopen architecture: %s\n' \
      "$architecture" >&2
    exit 2
    ;;
esac
...
work="$(mktemp -d "$temporary_root/a3s-oci-kvm-write-stdin-reopen.XXXXXX")"
current_stage="setup"
case_report=""
case_stderr=""
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${A3S_OCI_KEEP_FAILED_WORK:-0}" == "1" ]]; then
    printf 'preserving failed Linux KVM WriteStdin reopen work directory: %s\n' \
      "$work" >&2
    return 0
  fi
  case "$work" in
    "$temporary_root"/a3s-oci-kvm-write-stdin-reopen.*)
      rm -rf -- "$work"
      ;;
    *)
      printf 'refusing to clean unexpected Linux KVM WriteStdin reopen path: %s\n' \
        "$work" >&2
      return 1
      ;;
  esac
...
on_error() {
  local status=$?
  trap - ERR
  printf 'Linux KVM WriteStdin reopen stage %s failed with status %s near line %s\n' \
  ...
  if [[ -n "$case_report" && -s "$case_report" ]]; then
    jq --compact-output \
      '{schema_version, status, requested_stage, reason,
        ...
        write_stdin_journal_prepared_before_reopen,
        write_stdin_journal_succeeded_before_reopen,
        first_operation_dispatches, host_service_reopened,
        replacement_recovery_calls, replacement_rehydrated_created_record,
        replacement_rehydrated_running_record, replacement_rehydrated_exec_record,
        replacement_rehydrated_write_stdin, replacement_operation_dispatches,
        replacement_exec_pid, exec_response_rebound,
        ...
        replacement_exec_marker_verified, replacement_write_marker_verified,
...
report_path="${A3S_OCI_LINUX_KVM_WRITE_STDIN_REOPEN_REPORT:-$work/report.json}"
if [[ -e "$report_path" || -L "$report_path" ]]; then
  printf 'refusing to overwrite Linux KVM WriteStdin reopen report: %s\n' \
    "$report_path" >&2
  exit 1
fi
...
provenance="$(
  linux_kvm_provenance \
    linux-kvm-write-stdin-reopen-9-stage-v1 "$profile" \
...
if [[ "$kvm_status" == "unavailable" ]]; then
  ...
  jq --null-input \
    ...
    '{
      schema_version: "a3s.oci.linux-kvm-write-stdin-reopen-matrix.v1",
      platform: "linux",
  ...
  jq --exit-status \
    '.schema_version == "a3s.oci.linux-kvm-write-stdin-reopen-matrix.v1"
     ...
     and .provenance.qualification_profile
       == "linux-kvm-write-stdin-reopen-9-stage-v1"
...
scripts/prepare-utility-vm-bundle.sh \
  ...
  --bundle "$bundle" \
  --cgroups-path a3s-oci-kvm-write-stdin-reopen
init_marker="$bundle/rootfs/.a3s-oci-create-start-smoke"
exec_marker="$bundle/rootfs/.a3s-oci-exec-reopen-smoke"
write_marker="$bundle/rootfs/.a3s-oci-write-stdin-reopen-smoke"
test ! -e "$init_marker"
test ! -e "$exec_marker"
test ! -e "$write_marker"
...
run_stage() {
  ...
  printf 'Qualifying Linux KVM WriteStdin reopen stage: %s\n' "$stage" >&2

  set +e
  "$cli" linux-kvm-write-stdin-reopen \
    --shim "$shim" \
  ...
  jq --exit-status \
    ...
    '.schema_version
     ...
     and .bundle_loaded and .requested_operation == "write-stdin"
     and (.signal_process_signal == null)
     and (.write_stdin_data | length) > 0
     and .write_stdin_data[-1] == 10
     and (.exec_terminal | not)
     and (.exec_process_id | startswith("stdin-worker-"))
     and .requested_stage == $stage
     and (.qualification_operation_id | startswith("kvm-write-stdin-reopen-"))
     and (.setup_create_operation_id | startswith("kvm-write-stdin-reopen-"))
     and (.setup_start_operation_id | startswith("kvm-write-stdin-reopen-"))
     and (.setup_exec_operation_id | startswith("kvm-write-stdin-reopen-"))
     ...
     and (.container_id | startswith("kvm-write-stdin-reopen-"))
     and .negotiated_protocol == 10
     and .injected_point == ("agent-v10.write-stdin-" + $stage)
     ...
     and (if $stage == "guest-after-response-write" then
            ...
            and (.write_stdin_journal_prepared_before_reopen | not)
            and .write_stdin_journal_succeeded_before_reopen
            and .replacement_rehydrated_write_stdin
            and .operation_replayed_without_driver_dispatch
          else
            (.first_response_matches_durable_record | not)
            and .write_stdin_journal_prepared_before_reopen
            and (.write_stdin_journal_succeeded_before_reopen | not)
            and (.replacement_rehydrated_write_stdin | not)
     ...
     and .first_exec_marker_verified and .first_write_marker_verified
     ...
     and .exec_response_rebound and .exec_request_identity_reused
     and .write_stdin_request_identity_reused
     ...
     and .write_marker_reset_before_replacement
     and .replacement_write_marker_verified
     ...
     and .marker_absent_after_cleanup and .exec_marker_absent_after_cleanup
     and .write_marker_absent_after_cleanup
