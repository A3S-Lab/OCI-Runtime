run_filesystem_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/filesystem/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "filesystem"
       and (.file_op == null)
       and (.file_path == null)
       and (.file_data == null)
       and (.file_user == null)
       ...
       and (.filesystem_path
            | startswith("/tmp/.a3s-oci-filesystem-reopen-"))
       and (.filesystem_destination == null)
       and .filesystem_depth == 0
       and (.filesystem_user == null)
       ...
       and (.container_id | startswith("smoke-filesystem-reopen-"))
       and .negotiated_protocol == 10
       and .injected_point == ("agent-v10.filesystem-" + $stage)
       ...
       and .filesystem_effect_absent_after_cleanup
