run_write_stdin_stage() {
  local fault_stage="$1"
  local stage_console_dir="$console_root/write-stdin/$fault_stage"
  ...
  if hardware_available; then
    ...
    jq --exit-status --arg stage "$fault_stage" \
      '.schema_version
       ...
       and .bundle_loaded
       and .requested_operation == "write-stdin"
       ...
       and (.container_id | startswith("smoke-write-stdin-reopen-"))
       ...
       and .first_write_marker_verified
