...
use a3s_oci_sdk::{
    ContainerId, ContainerTarget, DeleteMode, DeleteRequest as RuntimeDeleteRequest, ErrorCode,
    Generation, LocalIpcEndpoint, OperationContext, OperationId, RuntimeClient, StateRequest,
};
use prost_types::Any;
...
pub(crate) async fn qualify_shim_sigkill(
    ...
    qualify_delete_effect_committed_shim_sigkill(config, prefix).await?;
    for stage in [
...
async fn qualify_delete_effect_committed_shim_sigkill(
    config: &QualificationConfig,
    prefix: &str,
) -> TestResult<()> {
