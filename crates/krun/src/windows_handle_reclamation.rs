//! Same-process WHPX lifecycle qualification.
//!
//! A one-shot VM smoke can hide native-handle leaks when the shim exits.  This
//! gate deliberately performs a warmup and a bounded number of complete VM
//! entries in the *same* process, sampling the process handle inventory after
//! every entry and requiring the final count to return to the warmed baseline.
...
use crate::windows_bootstrap_console;
...
pub(crate) fn run(
    ...
    let mut report = WhpxHandleReclamationSmokeReport::initial(iterations);
    if iterations == 0 || iterations > MAX_ITERATIONS {
        report.reason = Some(format!(
            "WHPX handle-reclamation iterations must be between 1 and {MAX_ITERATIONS}; requested {iterations}"
        ));
        return report;
    }
    ...
    if paths_overlap(&rootfs, &runtime_share)
        || paths_overlap(&console_directory, &runtime_share)
        || paths_overlap(&runtime_share, &console_directory)
    {
        return failed(
            report,
            "rootfs, console evidence, and writable runtime share must be disjoint".into(),
        );
    }
    ...
    if paths_overlap(&runtime_share, system_image.image_path())
        ...
        || paths_overlap(&rootfs, system_image.image_path())
    {
        return failed(
            report,
            "the immutable Windows system image and mutable qualification paths must be disjoint"
                .into(),
        );
    }
    ...
    // libkrun's Windows build normally transfers control to its init wrapper.
    // The qualification build exposes this opt-in return path so the same
    // shim can create the next context after each completed VM.
    ...
    if final_count > baseline.saturating_add(report.allowed_final_handle_delta) {
        report.reason = Some(format!(
            "WHPX native handles did not return to the warmed baseline: baseline={baseline}, peak={}, final={final_count}, allowed_delta={}",
...
fn run_cycle(
    ...
    let command = format!(
        "mkdir -p /run/a3s-oci-runtime && mount -t virtiofs {AGENT_RUNTIME_SHARE_TAG} /run/a3s-oci-runtime && printf '%s\\n' '{WORKLOAD_TOKEN}' > '{marker_guest_path}' && sync && printf '%s\\n' '{WORKLOAD_TOKEN}'"
    );
    ...
    if let Err(reason) = windows_bootstrap_console::merge(rootfs, &console_file) {
    ...
    match enter_result {
        ...
        Err(error) => {
            sample.reason.get_or_insert_with(|| error.to_string());
        }
    }
    ...
    if marker.exists() {
        match remove_marker(&marker) {
            ...
            Err(error) => {
                sample.reason.get_or_insert(error);
            }
        }
    }
...
fn current_process_handle_count() -> Result<u32, String> {
    ...
    if succeeded == 0 {
        ...
        Err("Windows reported an empty in-process handle inventory".into())
    } else {
        Ok(count)
    }
}
...
fn remove_marker(path: &Path) -> Result<(), String> {
    let metadata = fs::symlink_metadata(path).map_err(|error| {
        format!(
            "failed to inspect reclamation marker {}: {error}",
            path.display()
        )
    })?;
    if metadata.file_type().is_symlink()
    ...
    {
        return Err(format!(
            "reclamation marker is not a regular file: {}",
            path.display()
        ));
    }
    fs::remove_file(path).map_err(|error| {
        format!(
            "failed to remove reclamation marker {}: {error}",
            path.display()
        )
    })
}
