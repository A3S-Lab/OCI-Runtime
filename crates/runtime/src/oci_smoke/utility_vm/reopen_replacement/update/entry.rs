...
use a3s_oci_sdk::{
    CreateAttachments, CreateRequest, IoMode, IsolationRequest, OciBundle, OperationContext,
    ProcessIo, StartRequest, UpdateRequest,
};
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_update(HostPlatform::current(), stage);
    ...
    match path_exists(&marker).await {
        ...
        Ok(true) => {
            return failed(
                report,
                format!(
                    "refusing to overwrite an existing Update reopen qualification marker: {}",
                    marker.display()
                ),
            );
        }
    ...
    let exact_target = match target(&format!("update-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("update-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("update-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let update_operation_id = match operation_id(&format!("update-reopen-{nonce}-update")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("update-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id = match operation_id(&format!("update-reopen-{nonce}-stale-guest"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_host_operation_id = match operation_id(&format!("update-reopen-{nonce}-stale-host")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct Update Create attachments: {error}"),
            );
        }
    };
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest Update qualification: {error}"),
                );
            }
        }
    } else {
    ...
    report.qualification_operation_id = Some(update_operation_id);
    ...
    let state_root = console_directory.join(format!("a3s-oci-update-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory.join(format!("a3s-oci-update-reopen-{nonce}-first.log")),
        replacement_console: console_directory
            .join(format!("a3s-oci-update-reopen-{nonce}-replacement.log")),
        marker,
    ...
    match remove_marker_if_present(&qualification.marker).await {
        ...
        Ok(()) => append_reason(&mut report, "Update marker remained after cleanup"),
