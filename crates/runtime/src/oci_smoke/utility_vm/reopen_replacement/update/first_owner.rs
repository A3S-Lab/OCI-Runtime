...
pub(super) async fn run(
    ...
    .await
    {
        ...
        Ok(Ok(record)) => append_failure(
            &mut first_failure,
            format!("first Update unexpectedly completed before owner replacement: {record:?}"),
        ),
    ...
    .await
    {
        ...
        Ok(UpdateJournalStatus::Succeeded(journal)) => {
            ...
            if !report.first_response_matches_durable_record {
                append_failure(
                    &mut first_failure,
                    "completed Update Host journal did not match its durable record",
                );
            }
            if !response_delivered {
                append_failure(
                    &mut first_failure,
                    "Update Host journal committed before its response boundary",
                );
            }
        }
