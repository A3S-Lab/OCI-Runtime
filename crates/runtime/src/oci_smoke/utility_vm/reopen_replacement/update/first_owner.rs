...
use super::support::{record_interruption, update_journal_status, UpdateJournalStatus};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge.reason.clone().unwrap_or_else(|| {
                "failed to launch the first Update qualification VM".to_string()
            });
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for Update: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Update setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Update setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Update setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Update setup Start returned invalid {} record with PID {:?} and paused={}",
                    record.state.status(),
                    record.state.pid(),
                    record.is_paused()
                ),
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Update setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Update setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    if let Err(reason) = wait_for_exact_marker(
        ...
        "first-owner Update init readiness",
    )
    .await
    {
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut first_failure,
            format!(
                "{} did not deliver its completed Update response: {error}",
        ...
        Ok(Ok(record)) if response_delivered => append_failure(
            &mut first_failure,
            format!(
                "delivered Update response returned invalid {} record with PID {:?} and paused={}",
                record.state.status(),
                record.state.pid(),
                record.is_paused()
            ),
        ...
        Ok(Ok(_)) => append_failure(
            &mut first_failure,
            "first Update unexpectedly completed before owner replacement",
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first Update exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    report.first_operation_dispatches = driver.update_calls();
    ...
    let durable = match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained || report.durable_paused_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted Update did not retain the exact unpaused running state",
                );
            }
            Some(record)
        }
        Ok(records) => {
            append_failure(
                &mut first_failure,
                format!(
                    "interrupted Update retained {} records instead of one",
                    records.len()
                ),
            );
            None
        ...
        Err(error) => {
            append_failure(
                &mut first_failure,
                format!("failed to inspect state after interrupted Update: {error}"),
            );
            None
        }
    };
    ...
    .await
    {
        Ok(UpdateJournalStatus::Prepared) => {
            ...
            if response_delivered {
                append_failure(
                    &mut first_failure,
                    "delivered Update response left its journal prepared",
                );
            }
        }
        Ok(UpdateJournalStatus::Succeeded(journal)) => {
            ...
            if !report.first_response_matches_durable_record {
                append_failure(
                    &mut first_failure,
                    "completed Update journal did not match its response and durable record",
                );
            }
            if !response_delivered {
                append_failure(
                    &mut first_failure,
                    "Update journal succeeded without a delivered response",
                );
            }
        }
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest Update evidence did not match the qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut first_failure, reason),
    ...
    for (label, actual) in [
        ...
        ("Update", report.first_operation_dispatches),
    ] {
        if actual != 1 {
            append_failure(
                &mut first_failure,
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected Update transport point crossed {} times instead of once",
    ...
    let update_identity = identity_or_expected(
        update_identity,
        &mut first_failure,
        DriverUpdateRequest {
