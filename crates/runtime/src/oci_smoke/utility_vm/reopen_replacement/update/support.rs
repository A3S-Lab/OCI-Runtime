...
pub(in crate::oci_smoke::utility_vm::reopen_replacement) enum UpdateJournalStatus {
    Prepared,
...
pub(in crate::oci_smoke::utility_vm::reopen_replacement) async fn update_journal_status(
    ...
    let contents = tokio::fs::read(&path).await.map_err(|error| {
        format!(
            "failed to read durable Update journal {}: {error}",
            path.display()
        )
    })?;
    let value: serde_json::Value = serde_json::from_slice(&contents).map_err(|error| {
        format!(
            "failed to decode durable Update journal {}: {error}",
            path.display()
        )
    })?;
    let expected_generation = serde_json::to_value(target.generation)
        .map_err(|error| format!("failed to encode expected Update generation: {error}"))?;
    let identity_matches = value
        ...
        && value.get("kind").and_then(serde_json::Value::as_str) == Some("update")
    ...
    if !identity_matches {
        return Err(format!(
            "durable Update journal {} did not match the exact operation and generation",
            path.display()
        ));
    }
    let outcome = value
        .get("outcome")
        .ok_or_else(|| format!("durable Update journal {} has no outcome", path.display()))?;
    match outcome.get("status").and_then(serde_json::Value::as_str) {
        Some("prepared") => Ok(UpdateJournalStatus::Prepared),
        Some("succeeded") => {
            let response: ContainerRecord =
                serde_json::from_value(outcome.get("response").cloned().ok_or_else(|| {
                    format!(
                        "durable Update journal {} has no container response",
                        path.display()
                    )
                })?)
                .map_err(|error| {
                    format!(
                        "failed to decode durable Update response {}: {error}",
                        path.display()
                    )
                })?;
            ...
            {
                return Err(format!(
                    "durable Update response {} changed its exact target",
                    path.display()
                ));
            }
            Ok(UpdateJournalStatus::Succeeded(Box::new(response)))
        }
        status => Err(format!(
            "durable Update journal {} had unexpected status {status:?}",
            path.display()
        )),
    }
}
