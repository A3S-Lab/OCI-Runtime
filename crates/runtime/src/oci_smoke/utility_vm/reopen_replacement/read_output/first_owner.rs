...
use super::support::record_interruption;
use super::{FirstOwnerEvidence, Qualification, QualificationHvfDriver};
use crate::host_cleanup::MacosHostCleanupTracker;
use crate::{
    DriverExecRequest, DriverReadOutputRequest, OciVmOperationReopenReplacementReport,
    RuntimeDriver,
};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge.reason.clone().unwrap_or_else(|| {
                "failed to launch the first ReadOutput qualification VM".to_string()
            });
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for ReadOutput: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "ReadOutput setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("ReadOutput setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "ReadOutput setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "ReadOutput setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("ReadOutput setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "ReadOutput setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    .await
    {
        ...
        Ok(Ok(process)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("ReadOutput setup Exec returned invalid process {process:?}"),
            )
            .await;
        }
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("ReadOutput setup Exec failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "ReadOutput setup Exec exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    match (
        ...
        (Ok(ExecJournalStatus::Succeeded(_)), Ok(_)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "ReadOutput setup Exec journal changed its durable process".to_string(),
            )
            .await;
        }
        (Ok(ExecJournalStatus::Prepared), _) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "ReadOutput setup Exec journal remained prepared".to_string(),
            )
            .await;
        }
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut first_failure,
            format!(
                "{} did not deliver its completed ReadOutput response: {error}",
        ...
        Ok(Ok(chunks)) if response_delivered => {
            ...
            if !report.first_output_verified {
                append_failure(
                    &mut first_failure,
                    "delivered first ReadOutput response did not match the nonce-bound output",
                );
            }
        ...
        Ok(Ok(chunks)) => append_failure(
            &mut first_failure,
            format!("first ReadOutput unexpectedly completed before owner replacement: {chunks:?}"),
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first ReadOutput exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted ReadOutput changed the exact durable running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut first_failure,
            format!(
                "interrupted ReadOutput retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut first_failure,
            format!("failed to inspect state after interrupted ReadOutput: {error}"),
        ),
    }
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest ReadOutput evidence did not match the qualification",
                    );
                }
            }
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected ReadOutput transport point crossed {} times instead of once",
    ...
    let read_output_identity = identity_or_expected(
        read_output_identity,
        &mut first_failure,
        expected_driver_read,
    );
