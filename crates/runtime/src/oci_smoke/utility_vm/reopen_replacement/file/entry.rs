...
use a3s_oci_sdk::{
    ContainerTarget, CreateAttachments, CreateRequest, FileOp, FileRequest, FilesystemOp,
    FilesystemRequest, IoMode, IsolationRequest, OciBundle, OperationContext, ProcessIo,
    StartRequest,
};
...
use super::support::session_filesystem_bundle;
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_file(HostPlatform::current(), stage);
    ...
    let bundle = match nonce_bound_bundle(bundle, &nonce).and_then(session_filesystem_bundle) {
        Ok(bundle) => {
    ...
    match path_exists(&init_marker).await {
        ...
        Ok(true) => {
            return failed(
                report,
                format!(
                    "refusing to overwrite an existing File init marker: {}",
                    init_marker.display()
                ),
            );
        }
    ...
    let exact_target = match target(&format!("file-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("file-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("file-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let file_operation_id = match operation_id(&format!("file-reopen-{nonce}-upload")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let cleanup_operation_id = match operation_id(&format!("file-reopen-{nonce}-cleanup")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("file-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id = match operation_id(&format!("file-reopen-{nonce}-stale-guest")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_host_operation_id = match operation_id(&format!("file-reopen-{nonce}-stale-host")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct File Create attachments: {error}"),
            );
        }
    };
    ...
    let cleanup_file = FilesystemRequest {
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest File qualification: {error}"),
                );
            }
        }
    } else {
    ...
    let state_root = console_directory.join(format!("a3s-oci-file-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory.join(format!("a3s-oci-file-reopen-{nonce}-first.log")),
        replacement_console: console_directory
            .join(format!("a3s-oci-file-reopen-{nonce}-replacement.log")),
        init_marker,
    ...
    match remove_marker_if_present(&qualification.init_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "File init marker remained after cleanup"),
