...
use a3s_oci_sdk::{FileRequest, ListRequest, OciRuntimeService, StateRequest};
...
use super::support::{record_interruption, upload_response_matches};
...
pub(super) async fn run(
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut first_failure,
            format!(
                "{} did not deliver its completed File response: {error}",
                qualification.stage.as_str()
            ),
        ),
        Ok(Ok(response)) if response_delivered => {
            report.first_operation_response_received = true;
            report.first_file_response_verified = upload_response_matches(
                &response,
                &qualification.start.target,
                qualification.expected_payload.len(),
            );
            if !report.first_file_response_verified {
                append_failure(
                    &mut first_failure,
                    format!("first File upload returned an invalid response: {response:?}"),
                );
            }
