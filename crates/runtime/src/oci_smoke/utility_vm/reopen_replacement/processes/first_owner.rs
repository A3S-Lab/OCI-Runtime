...
use super::super::exec::support::{
    durable_exec_process, exact_process_target, exec_journal_status, identity_or_expected,
    reset_marker, shutdown_setup_failure, wait_for_exact_marker, ExecJournalStatus,
};
use super::super::{append_failure, QUALIFICATION_TIMEOUT};
use super::support::{inventory_matches, record_interruption};
use super::{FirstOwnerEvidence, Qualification, QualificationHvfDriver};
use crate::host_cleanup::MacosHostCleanupTracker;
use crate::{DriverExecRequest, OciVmOperationReopenReplacementReport, RuntimeDriver};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge.reason.clone().unwrap_or_else(|| {
                "failed to launch the first Processes qualification VM".to_string()
            });
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for Processes: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Processes setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Processes setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Processes setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Processes setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Processes setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Processes setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    .await
    {
        ...
        Ok(Ok(process)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Processes setup Exec returned invalid process {process:?}"),
            )
            .await;
        }
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Processes setup Exec failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Processes setup Exec exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    match (
        ...
        (Ok(ExecJournalStatus::Succeeded(_)), Ok(_)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "Processes setup Exec journal changed its durable process".to_string(),
            )
            .await;
        }
        (Ok(ExecJournalStatus::Prepared), _) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "Processes setup Exec journal remained prepared".to_string(),
            )
            .await;
        }
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut first_failure,
            format!(
                "{} did not deliver its completed Processes response: {error}",
        ...
        Ok(Ok(inventory)) if response_delivered => {
            ...
            if !report.first_process_inventory_verified {
                append_failure(
                    &mut first_failure,
                    "delivered first Processes response did not contain the exact init and Exec inventory",
                );
            }
        ...
        Ok(Ok(inventory)) => append_failure(
            &mut first_failure,
            format!(
                "first Processes unexpectedly completed before owner replacement: {inventory:?}"
            ),
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first Processes exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted Processes query changed the exact durable running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut first_failure,
            format!(
                "interrupted Processes query retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut first_failure,
            format!("failed to inspect state after interrupted Processes: {error}"),
        ),
    }
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest Processes evidence did not match the qualification",
                    );
                }
            }
    ...
    for (label, actual) in [
        ...
        ("Processes", report.first_operation_dispatches),
    ] {
        if actual != 1 {
            append_failure(
                &mut first_failure,
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected Processes transport point crossed {} times instead of once",
    ...
    let processes_identity = identity_or_expected(
        processes_identity,
        &mut first_failure,
        qualification.start.target.clone(),
    );
