...
use a3s_oci_sdk::{
    ...
    OciBundle, OperationContext, ProcessIo, ProcessesRequest, StartRequest,
};
...
use super::super::exec::support::{exec_process, nonce_bound_bundle, operation_id};
use super::super::exec::EXEC_MARKER_NAME;
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_processes(HostPlatform::current(), stage);
        ...
        match path_exists(marker).await {
            ...
            Ok(true) => {
                return failed(
                    report,
                    format!(
                        "refusing to overwrite an existing Processes {label} marker: {}",
                        marker.display()
                    ),
                );
            }
    ...
    let exact_target = match target(&format!("processes-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let qualification_operation_id = match operation_id(&format!("processes-reopen-{nonce}-query"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("processes-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("processes-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("processes-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("processes-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct Processes Create attachments: {error}"),
            );
        }
    };
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest Processes qualification: {error}"),
                );
            }
        }
    } else {
    ...
    let state_root = console_directory.join(format!("a3s-oci-processes-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory
            .join(format!("a3s-oci-processes-reopen-{nonce}-first.log")),
        replacement_console: console_directory
            .join(format!("a3s-oci-processes-reopen-{nonce}-replacement.log")),
        init_marker,
        exec_marker,
    ...
    match remove_marker_if_present(&qualification.init_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "Processes init marker remained after cleanup"),
    ...
    match remove_marker_if_present(&qualification.exec_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "Processes Exec marker remained after cleanup"),
        Err(reason) => append_reason(&mut report, reason),
    }
