...
use a3s_oci_sdk::{
    ...
    OperationContext, ProcessIo, ProcessTarget, Signal, SignalProcessRequest, StartRequest,
};
...
use super::support::signalable_exec_process;
use super::{exercise, Qualification, SIGNAL_MARKER_NAME, SIGNAL_PROCESS_SIGNAL};
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let signal_marker = rootfs.join(SIGNAL_MARKER_NAME);
    for (label, marker) in [
        ("init", &init_marker),
        ("Exec", &exec_marker),
    ...
    let exact_target = match target(&format!("signal-process-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("signal-process-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("signal-process-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("signal-process-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let signal_operation_id = match operation_id(&format!("signal-process-reopen-{nonce}-signal")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("signal-process-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id =
        match operation_id(&format!("signal-process-reopen-{nonce}-stale-guest")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let stale_host_operation_id =
        match operation_id(&format!("signal-process-reopen-{nonce}-stale-host")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let (process_id, process, process_io) = match signalable_exec_process(&nonce) {
        Ok(process) => process,
        Err(reason) => return failed(report, reason),
    };
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct SignalProcess Create attachments: {error}"),
            );
        }
    };
    ...
    let signal_process = SignalProcessRequest {
        context: OperationContext::new(signal_operation_id.clone()),
        process: ProcessTarget {
            container: exact_target.clone(),
            process_id: process_id.clone(),
        },
        signal: match Signal::new(SIGNAL_PROCESS_SIGNAL) {
            Ok(signal) => signal,
            Err(error) => {
                return failed(
                    report,
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest SignalProcess qualification: {error}"),
                );
            }
        }
    } else {
    ...
    report.qualification_operation_id = Some(signal_operation_id);
    ...
    let state_root = console_directory.join(format!("a3s-oci-signal-process-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory
            .join(format!("a3s-oci-signal-process-reopen-{nonce}-first.log")),
        replacement_console: console_directory.join(format!(
            "a3s-oci-signal-process-reopen-{nonce}-replacement.log"
        )),
        init_marker,
        exec_marker,
        signal_marker,
    ...
    match remove_marker_if_present(&qualification.init_marker).await {
        ...
        Ok(()) => append_reason(
            &mut report,
            "SignalProcess init marker remained after cleanup",
        ),
    ...
    match remove_marker_if_present(&qualification.exec_marker).await {
        ...
        Ok(()) => append_reason(
            &mut report,
            "SignalProcess Exec marker remained after cleanup",
        ),
        Err(reason) => append_reason(&mut report, reason),
    }
    match remove_marker_if_present(&qualification.signal_marker).await {
        Ok(()) if path_exists(&qualification.signal_marker).await == Ok(false) => {
            report.signal_marker_absent_after_cleanup = true;
        }
        Ok(()) => append_reason(
            &mut report,
            "SignalProcess signal marker remained after cleanup",
        ),
        Err(reason) => append_reason(&mut report, reason),
    }
