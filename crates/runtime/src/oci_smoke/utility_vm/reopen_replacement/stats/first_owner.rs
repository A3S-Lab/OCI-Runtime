...
use crate::{DriverUpdateRequest, OciVmOperationReopenReplacementReport, RuntimeDriver};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge
                .reason
                .clone()
                .unwrap_or_else(|| "failed to launch the first Stats qualification VM".to_string());
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for Stats: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Stats setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Stats setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Stats setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Stats setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Stats setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Stats setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    let first_update = match timeout(
        QUALIFICATION_TIMEOUT,
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Stats setup Update returned invalid {} record with PID {:?} and paused={}",
                    record.state.status(),
                    record.state.pid(),
                    record.is_paused()
                ),
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Stats setup Update failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Stats setup Update exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    .await
    {
        ...
        Ok(UpdateJournalStatus::Succeeded(_)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "Stats setup Update journal changed its durable response".to_string(),
            )
            .await;
        }
        Ok(UpdateJournalStatus::Prepared) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "Stats setup Update journal remained prepared".to_string(),
            )
            .await;
        }
        Err(reason) => {
    ...
    .await
    {
        ...
        Ok(Ok(stats)) if response_delivered => {
            ...
            if !report.first_stats_verified {
                append_failure(
                    &mut first_failure,
                    "delivered first Stats response did not match the exact updated resource profile",
                );
            }
        ...
        Ok(Ok(stats)) => append_failure(
            &mut first_failure,
            format!("first Stats unexpectedly completed before owner replacement: {stats:?}"),
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first Stats exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted Stats query changed the exact durable running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut first_failure,
            format!(
                "interrupted Stats query retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut first_failure,
            format!("failed to inspect state after interrupted Stats: {error}"),
        ),
    }

    let expected_driver_update = DriverUpdateRequest {
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest Stats evidence did not match the qualification",
                    );
                }
            }
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected Stats transport point crossed {} times instead of once",
    ...
    let update_identity =
        identity_or_expected(update_identity, &mut first_failure, expected_driver_update);
    let stats_identity = identity_or_expected(
        stats_identity,
        &mut first_failure,
        qualification.start.target.clone(),
    );
