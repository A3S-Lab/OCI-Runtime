...
use super::support::{record_recovery_evidence, resize_journal_status, ResizeJournalStatus};
...
pub(super) async fn run(
    ...
    .await
    {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge.reason.clone().unwrap_or_else(|| {
                "failed to launch the replacement Resize qualification VM".to_string()
            });
            report.replacement_vm = bridge;
            return Err(reason);
        }
    };
    let recovery_resize = response_delivered.then(|| qualification.resize.clone());
    let recovery_resize_ready_marker = response_delivered.then(|| {
        (
            qualification.exec_marker.clone(),
            qualification.exec_marker_contents.clone(),
        )
    });
    let driver = Arc::new(QualificationHvfDriver::with_resize_recovery(
        Arc::clone(&session),
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to reopen durable Host service around replacement Resize VM: {error}"
            ));
        }
    };
    ...
    if report.replacement_rehydrated_resize != response_delivered {
        append_failure(
            &mut failure,
            "replacement recovery terminal resize did not match the durable journal outcome",
        );
    }
    ...
    match service.list(ListRequest::default()).await {
        ...
        Ok(records) => append_failure(
            &mut failure,
            format!(
                "replacement Resize recovery retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut failure,
            format!("failed to inspect recovered Resize record: {error}"),
        ),
    }
    ...
    if !response_delivered {
        match super::super::super::path_exists(&qualification.resize_marker).await {
            Ok(false) => {}
            Ok(true) => append_failure(
                &mut failure,
                "replacement recovery dispatched an uncommitted Resize before API retry",
            ),
            Err(reason) => append_failure(&mut failure, reason),
        }
    }
    let calls_before_write = driver.resize_calls();
    match timeout(
        QUALIFICATION_TIMEOUT,
        service.resize(qualification.resize.clone()),
    )
    .await
    {
        Ok(Ok(())) => {
            report.operation_completed_after_reopen = true;
            report.generation_after_reopen = qualification.resize.process.container.generation;
            ...
            if !report.same_generation_reused {
                append_failure(
                    &mut failure,
                    "replacement Resize did not retain the durable generation",
                );
            }
        }
        Ok(Err(error)) => {
            append_failure(&mut failure, format!("replacement Resize failed: {error}"))
        }
        Err(_) => append_failure(
            &mut failure,
            format!(
                "replacement Resize exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    report.operation_replayed_without_driver_dispatch = driver.resize_calls() == calls_before_write;
    match wait_for_exact_marker(
        &qualification.resize_marker,
    ...
    match resize_journal_status(
        &qualification.state_root,
        &qualification.resize.context.operation_id,
        &qualification.resize.process,
    )
    .await
    {
        ...
        Ok(ResizeJournalStatus::Prepared) => {
            append_failure(&mut failure, "replacement Resize journal remained prepared")
        }
        Err(reason) => append_failure(&mut failure, reason),
    }
    if let Some(process) = replacement_exec.as_ref() {
        match durable_exec_process(&qualification.state_root, &exact_process).await {
            Ok(durable) => {
                ...
                if !report.exec_response_rebound {
                    append_failure(
                        &mut failure,
                        "completed Resize did not release the rebound Exec record",
                    );
                }
            }
            Err(reason) => append_failure(&mut failure, reason),
    ...
    .await
    {
        ...
        Ok(Ok(())) => append_failure(
            &mut failure,
            "later Resize replay reached the replacement driver",
        ),
        Ok(Err(error)) => {
            append_failure(&mut failure, format!("later Resize replay failed: {error}"))
        }
        Err(_) => append_failure(&mut failure, "later Resize replay timed out"),
    }
    ...
    if report.operation_replayed_without_driver_dispatch != response_delivered {
        append_failure(
            &mut failure,
            "replacement Resize dispatch did not match the journal outcome",
        );
    }
    if report.replacement_operation_dispatches != 1 {
        append_failure(
            &mut failure,
            format!(
                "replacement driver recorded {} Resize dispatches instead of one",
    ...
    match driver.resize_identity() {
        Ok(identity) => {
            ...
            report.same_operation_id_reused = report.resize_request_identity_reused
                && identity.context.operation_id == qualification.resize.context.operation_id
                && identity.target == exact_process
                && identity.size == qualification.resize.size;
            if !report.resize_request_identity_reused || !report.same_operation_id_reused {
                append_failure(
                    &mut failure,
                    "replacement Resize changed its operation, target, or size identity",
                );
            }
        }
        Err(reason) => append_failure(&mut failure, reason),
    ...
    match service.resize(changed_host).await {
        ...
        Err(error) => append_failure(
            &mut failure,
            format!("reopened Host returned the wrong changed Resize error: {error}"),
        ),
    ...
    match timeout(
        ...
        session.client().resize(AgentResizeRequest {
            ...
            size: qualification.resize.size,
        }),
    )
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("replacement Guest returned the wrong stale Resize error: {error}"),
        ),
        Ok(Ok(())) => append_failure(&mut failure, "replacement Guest accepted stale Resize"),
        Err(_) => append_failure(
            &mut failure,
            "replacement Guest stale Resize check timed out",
        ),
    }
        ...
        .await
    {
        ...
        Err(error) => append_failure(
            &mut failure,
            format!("reopened Host returned the wrong stale Resize error: {error}"),
        ),
        Ok(()) => append_failure(&mut failure, "reopened Host accepted stale Resize"),
    }
