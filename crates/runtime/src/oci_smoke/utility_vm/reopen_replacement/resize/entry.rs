...
use a3s_oci_sdk::{
    ...
    OperationContext, ProcessIo, ProcessTarget, ResizeRequest, StartRequest, TerminalSize,
};
...
use super::support::terminal_exec_process;
use super::{exercise, Qualification, RESIZE_MARKER_NAME};
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_resize(HostPlatform::current(), stage);
    ...
    let resize_marker = rootfs.join(RESIZE_MARKER_NAME);
    for (label, marker) in [
        ("init", &init_marker),
        ("Exec", &exec_marker),
    ...
    let exact_target = match target(&format!("resize-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("resize-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("resize-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("resize-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let resize_operation_id = match operation_id(&format!("resize-reopen-{nonce}-resize")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("resize-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id = match operation_id(&format!("resize-reopen-{nonce}-stale-guest"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_host_operation_id = match operation_id(&format!("resize-reopen-{nonce}-stale-host")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let (process_id, process, process_io) = match terminal_exec_process(&nonce) {
        Ok(process) => process,
        Err(reason) => return failed(report, reason),
    };
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct Resize Create attachments: {error}"),
            );
        }
    };
    ...
    let resize = ResizeRequest {
        context: OperationContext::new(resize_operation_id.clone()),
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest Resize qualification: {error}"),
                );
            }
        }
    } else {
    ...
    report.qualification_operation_id = Some(resize_operation_id);
    ...
    report.resize_size = Some(resize.size);

    let state_root = console_directory.join(format!("a3s-oci-resize-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory.join(format!("a3s-oci-resize-reopen-{nonce}-first.log")),
        replacement_console: console_directory
            .join(format!("a3s-oci-resize-reopen-{nonce}-replacement.log")),
        init_marker,
        exec_marker,
        resize_marker,
    ...
    match remove_marker_if_present(&qualification.init_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "Resize init marker remained after cleanup"),
    ...
    match remove_marker_if_present(&qualification.exec_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "Resize Exec marker remained after cleanup"),
        Err(reason) => append_reason(&mut report, reason),
    }
    match remove_marker_if_present(&qualification.resize_marker).await {
        Ok(()) if path_exists(&qualification.resize_marker).await == Ok(false) => {
            report.resize_marker_absent_after_cleanup = true;
        }
        Ok(()) => append_reason(&mut report, "Resize effect marker remained after cleanup"),
        Err(reason) => append_reason(&mut report, reason),
    }
