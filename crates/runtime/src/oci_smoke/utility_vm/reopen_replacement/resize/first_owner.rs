...
use super::support::{
    identity_or_expected, record_interruption, resize_journal_status, verify_first_resize_marker,
    ResizeJournalStatus,
};
use super::{FirstOwnerEvidence, Qualification, QualificationHvfDriver};
use crate::host_cleanup::MacosHostCleanupTracker;
use crate::{
    DriverExecRequest, DriverResizeRequest, OciVmOperationReopenReplacementReport, RuntimeDriver,
};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge.reason.clone().unwrap_or_else(|| {
                "failed to launch the first Resize qualification VM".to_string()
            });
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for Resize: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Resize setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Resize setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Resize setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Resize setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Resize setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Resize setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    .await
    {
        ...
        Ok(Ok(process)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Resize setup Exec returned invalid process {process:?}"),
            )
            .await;
        }
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Resize setup Exec failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Resize setup Exec exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    match (
        ...
        (Ok(ExecJournalStatus::Succeeded(_)), Ok(_)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "Resize setup Exec journal changed its durable process".to_string(),
            )
            .await;
        }
        (Ok(ExecJournalStatus::Prepared), _) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "Resize setup Exec journal remained prepared".to_string(),
            )
            .await;
        }
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut first_failure,
            format!(
                "{} did not deliver its completed Resize response: {error}",
        ...
        Ok(Ok(())) => append_failure(
            &mut first_failure,
            "first Resize unexpectedly completed before owner replacement",
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first Resize exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted Resize did not retain the exact running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut first_failure,
            format!(
                "interrupted Resize retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut first_failure,
            format!("failed to inspect state after interrupted Resize: {error}"),
        ),
    }
    ...
    .await
    {
        Ok(ResizeJournalStatus::Prepared) => {
            ...
            if response_delivered {
                append_failure(
                    &mut first_failure,
                    "delivered Resize response left its journal prepared",
                );
            }
        }
        Ok(ResizeJournalStatus::SucceededEmpty) => {
            ...
            if !response_delivered {
                append_failure(
                    &mut first_failure,
                    "Resize journal succeeded without a delivered response",
                );
            }
        }
        Err(reason) => append_failure(&mut first_failure, reason),
    ...
    let expected_driver_resize = DriverResizeRequest {
        context: qualification.resize.context.clone(),
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest Resize evidence did not match the qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut first_failure, reason),
    ...
    match reset_marker(&qualification.resize_marker).await {
        Ok(()) => report.resize_marker_reset_before_replacement = true,
        Err(reason) => append_failure(&mut first_failure, reason),
    }
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected Resize transport point crossed {} times instead of once",
    ...
    let resize_identity =
        identity_or_expected(resize_identity, &mut first_failure, expected_driver_resize);
