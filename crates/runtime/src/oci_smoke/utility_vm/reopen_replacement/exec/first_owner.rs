...
use super::support::{
    exact_process_target, exec_journal_status, identity_or_expected, record_interruption,
    reset_marker, shutdown_setup_failure, verify_first_exec_marker, wait_for_exact_marker,
    ExecJournalStatus,
};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge
                .reason
                .clone()
                .unwrap_or_else(|| "failed to launch the first Exec qualification VM".to_string());
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for Exec: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Exec setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Exec setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Exec setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Exec setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("Exec setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "Exec setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    .await
    {
        ...
        Ok(Ok(process)) => append_failure(
            &mut first_failure,
            format!("first Exec unexpectedly completed before owner replacement: {process:?}"),
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first Exec exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted Exec did not retain the exact durable running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut first_failure,
            format!(
                "interrupted Exec retained {} durable records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut first_failure,
            format!("failed to inspect durable state after interrupted Exec: {error}"),
        ),
    }
    ...
    .await
    {
        Ok(ExecJournalStatus::Prepared) => {
            ...
            if response_delivered {
                append_failure(
                    &mut first_failure,
                    "delivered first Exec response left its journal prepared",
                );
            }
        }
    ...
    let expected_driver_exec = DriverExecRequest {
        ...
        target: expected_process_target,
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest Exec evidence did not match the exact qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut first_failure, reason),
    ...
    if driver.start_calls() != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "first driver recorded {} Start setup dispatches instead of one",
                driver.start_calls()
            ),
        );
    }
    if report.first_operation_dispatches != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "first driver recorded {} Exec dispatches instead of one",
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected Exec transport point crossed {} times instead of once",
