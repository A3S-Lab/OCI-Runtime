...
use a3s_oci_sdk::{
    ...
    OperationContext, ProcessIo, ProcessTarget, StartRequest, WriteStdinRequest,
};
...
use super::{exercise, Qualification, WRITE_MARKER_NAME};
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_write_stdin(HostPlatform::current(), stage);
    ...
    let write_marker = rootfs.join(WRITE_MARKER_NAME);
    for (label, marker) in [
        ("init", &init_marker),
        ("Exec", &exec_marker),
    ...
    let exact_target = match target(&format!("write-stdin-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("write-stdin-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("write-stdin-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("write-stdin-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let write_operation_id = match operation_id(&format!("write-stdin-reopen-{nonce}-write")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("write-stdin-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id =
        match operation_id(&format!("write-stdin-reopen-{nonce}-stale-guest")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let stale_host_operation_id =
        match operation_id(&format!("write-stdin-reopen-{nonce}-stale-host")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    ...
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct WriteStdin Create attachments: {error}"),
            );
        }
    };
    ...
    let write_stdin = WriteStdinRequest {
        context: OperationContext::new(write_operation_id.clone()),
        process: ProcessTarget {
            container: exact_target.clone(),
            process_id: process_id.clone(),
        },
        data: format!("a3s-oci-write-stdin-{nonce}\n").into_bytes(),
    };
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            write_operation_id.clone(),
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest WriteStdin qualification: {error}"),
                );
            }
        }
    } else {
    ...
    report.qualification_operation_id = Some(write_operation_id);
    ...
    report.write_stdin_data = Some(write_stdin.data.clone());

    let state_root = console_directory.join(format!("a3s-oci-write-stdin-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory
            .join(format!("a3s-oci-write-stdin-reopen-{nonce}-first.log")),
        replacement_console: console_directory.join(format!(
            "a3s-oci-write-stdin-reopen-{nonce}-replacement.log"
        )),
        init_marker,
        exec_marker,
        write_marker,
        ...
        write_marker_contents: format!("a3s-oci-write-stdin-{nonce}\n").into_bytes(),
        create,
        start,
        exec,
        write_stdin,
    ...
    match remove_marker_if_present(&qualification.init_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "WriteStdin init marker remained after cleanup"),
    ...
    match remove_marker_if_present(&qualification.exec_marker).await {
        ...
        Ok(()) => append_reason(&mut report, "WriteStdin Exec marker remained after cleanup"),
        Err(reason) => append_reason(&mut report, reason),
    }
    match remove_marker_if_present(&qualification.write_marker).await {
        Ok(()) if path_exists(&qualification.write_marker).await == Ok(false) => {
            report.write_marker_absent_after_cleanup = true;
        }
        Ok(()) => append_reason(
            &mut report,
            "WriteStdin effect marker remained after cleanup",
        ),
        Err(reason) => append_reason(&mut report, reason),
    }
