...
use super::support::{
    close_stdin_journal_status, identity_or_expected, record_interruption,
    verify_first_close_marker, CloseStdinJournalStatus,
};
use super::{FirstOwnerEvidence, Qualification, QualificationHvfDriver};
use crate::host_cleanup::MacosHostCleanupTracker;
use crate::{
    DriverCloseStdinRequest, DriverExecRequest, OciVmOperationReopenReplacementReport,
    RuntimeDriver,
};
...
pub(super) async fn run(
    ...
    let session = match session_result {
        ...
        Err(mut bridge) => {
            ...
            let reason = bridge.reason.clone().unwrap_or_else(|| {
                "failed to launch the first CloseStdin qualification VM".to_string()
            });
    ...
    .await
    {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to open the first durable Host service for CloseStdin: {error}"
            ));
        }
    };
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "CloseStdin setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("CloseStdin setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "CloseStdin setup Create exceeded the {} second timeout",
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "CloseStdin setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("CloseStdin setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "CloseStdin setup Start exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    .await
    {
        ...
        Ok(Ok(process)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("CloseStdin setup Exec returned invalid process {process:?}"),
            )
            .await;
        }
        Ok(Err(error)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                format!("CloseStdin setup Exec failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            return shutdown_setup_failure(
                ...
                report,
                format!(
                    "CloseStdin setup Exec exceeded the {} second timeout",
                    QUALIFICATION_TIMEOUT.as_secs()
                ),
            )
            .await;
    ...
    match (
        ...
        (Ok(ExecJournalStatus::Succeeded(_)), Ok(_)) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "CloseStdin setup Exec journal changed its durable process".to_string(),
            )
            .await;
        }
        (Ok(ExecJournalStatus::Prepared), _) => {
            return shutdown_setup_failure(
                ...
                cleanup,
                report,
                "CloseStdin setup Exec journal remained prepared".to_string(),
            )
            .await;
        }
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut first_failure,
            format!(
                "{} did not deliver its completed CloseStdin response: {error}",
        ...
        Ok(Ok(())) => append_failure(
            &mut first_failure,
            "first CloseStdin unexpectedly completed before owner replacement",
        ),
        Err(_) => append_failure(
            &mut first_failure,
            format!(
                "first CloseStdin exceeded the {} second timeout",
                QUALIFICATION_TIMEOUT.as_secs()
            ),
        ),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut first_failure,
                    "interrupted CloseStdin did not retain the exact running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut first_failure,
            format!(
                "interrupted CloseStdin retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut first_failure,
            format!("failed to inspect state after interrupted CloseStdin: {error}"),
        ),
    }
    ...
    .await
    {
        Ok(CloseStdinJournalStatus::Prepared) => {
            ...
            if response_delivered {
                append_failure(
                    &mut first_failure,
                    "delivered CloseStdin response left its journal prepared",
                );
            }
        }
        Ok(CloseStdinJournalStatus::SucceededEmpty) => {
            ...
            if !response_delivered {
                append_failure(
                    &mut first_failure,
                    "CloseStdin journal succeeded without a delivered response",
                );
            }
        }
        Err(reason) => append_failure(&mut first_failure, reason),
    ...
    let expected_driver_close = DriverCloseStdinRequest {
        context: qualification.close_stdin.context.clone(),
        ...
        match read_guest_qualification_evidence(&qualification.first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut first_failure,
                        "Guest CloseStdin evidence did not match the qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut first_failure, reason),
    ...
    match reset_marker(&qualification.close_marker).await {
        Ok(()) => report.close_marker_reset_before_replacement = true,
        Err(reason) => append_failure(&mut first_failure, reason),
    }
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut first_failure,
            format!(
                "selected CloseStdin transport point crossed {} times instead of once",
    ...
    let close_stdin_identity = identity_or_expected(
