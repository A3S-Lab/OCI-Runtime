...
use a3s_oci_sdk::{
    ...
    OciBundle, OperationContext, ProcessIo, ProcessTarget, Signal, SignalProcessRequest,
    StartRequest, WaitProcessRequest,
};
...
use super::support::waitable_exec_process;
...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_wait_process(HostPlatform::current(), stage);
        ...
        match path_exists(marker).await {
            ...
            Ok(true) => {
                return failed(
                    report,
                    format!(
                        "refusing to overwrite an existing WaitProcess {label} marker: {}",
                        marker.display()
                    ),
                );
            }
    ...
    let exact_target = match target(&format!("wait-process-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("wait-process-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("wait-process-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("wait-process-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let signal_operation_id = match operation_id(&format!("wait-process-reopen-{nonce}-signal")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let qualification_operation_id =
        match operation_id(&format!("wait-process-reopen-{nonce}-qualification")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let delete_operation_id = match operation_id(&format!("wait-process-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let (process_id, process, process_io) = match waitable_exec_process(&nonce) {
        Ok(process) => process,
        Err(reason) => return failed(report, reason),
    };
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct WaitProcess Create attachments: {error}"),
            );
        }
    };
    ...
    let signal_process = SignalProcessRequest {
        ...
        signal: match Signal::new(WAIT_PROCESS_SIGNAL) {
            Ok(signal) => signal,
            Err(error) => {
                return failed(
                    report,
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest WaitProcess qualification: {error}"),
                );
            }
        }
    } else {
    ...
    report.setup_signal_process_operation_id = Some(signal_operation_id);
    report.container_id = Some(exact_target.id.clone());
    report.exec_process_id = Some(process_id);

    let state_root = console_directory.join(format!("a3s-oci-wait-process-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory
            .join(format!("a3s-oci-wait-process-reopen-{nonce}-first.log")),
        replacement_console: console_directory.join(format!(
            "a3s-oci-wait-process-reopen-{nonce}-replacement.log"
        )),
        init_marker,
        exec_marker,
    ...
    match remove_marker_if_present(&qualification.init_marker).await {
        ...
        Ok(()) => append_reason(
            &mut report,
            "WaitProcess init marker remained after cleanup",
        ),
    ...
    match remove_marker_if_present(&qualification.exec_marker).await {
        ...
        Ok(()) => append_reason(
            &mut report,
            "WaitProcess Exec marker remained after cleanup",
        ),
        Err(reason) => append_reason(&mut report, reason),
    }
    match tokio::fs::remove_dir_all(&state_root).await {
        Ok(()) if path_exists(&state_root).await == Ok(false) => report.state_root_removed = true,
        Ok(()) => append_reason(
            &mut report,
            format!(
