...
pub(in crate::oci_smoke::utility_vm) async fn run(
    ...
    let mut report =
        OciVmOperationReopenReplacementReport::initial_pause(HostPlatform::current(), stage);
    ...
    match path_exists(&marker).await {
        ...
        Ok(true) => {
            return failed(
                report,
                format!(
                    "refusing to overwrite an existing Pause reopen qualification marker: {}",
                    marker.display()
                ),
            );
        }
    ...
    let exact_target = match target(&format!("pause-reopen-{nonce}")) {
        Ok(target) => target,
        Err(reason) => return failed(report, reason),
    };
    let create_operation_id = match operation_id(&format!("pause-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("pause-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let pause_operation_id = match operation_id(&format!("pause-reopen-{nonce}-pause")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("pause-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id = match operation_id(&format!("pause-reopen-{nonce}-stale-guest"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_host_operation_id = match operation_id(&format!("pause-reopen-{nonce}-stale-host")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let attachments = match CreateAttachments::from_bundle(
        ...
        Err(error) => {
            return failed(
                report,
                format!("failed to construct Pause Create attachments: {error}"),
            );
        }
    };
    ...
    let guest_qualification = if stage.is_guest() {
        match AgentTransportQualificationRequest::new(
            ...
            Err(error) => {
                return failed(
                    report,
                    format!("failed to construct Guest Pause qualification: {error}"),
                );
            }
        }
    } else {
    ...
    report.qualification_operation_id = Some(pause_operation_id);
    ...
    let state_root = console_directory.join(format!("a3s-oci-pause-reopen-{nonce}-state"));
    ...
    let qualification = Qualification {
        ...
        first_console: console_directory.join(format!("a3s-oci-pause-reopen-{nonce}-first.log")),
        replacement_console: console_directory
            .join(format!("a3s-oci-pause-reopen-{nonce}-replacement.log")),
        marker,
    ...
    match remove_marker_if_present(&qualification.marker).await {
        ...
        Ok(()) => append_reason(&mut report, "Pause marker remained after cleanup"),
