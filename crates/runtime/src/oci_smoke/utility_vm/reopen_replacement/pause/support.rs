...
pub(super) async fn pause_journal_status(
    ...
    let contents = tokio::fs::read(&path).await.map_err(|error| {
        format!(
            "failed to read durable Pause journal {}: {error}",
            path.display()
        )
    })?;
    let value: serde_json::Value = serde_json::from_slice(&contents).map_err(|error| {
        format!(
            "failed to decode durable Pause journal {}: {error}",
            path.display()
        )
    })?;
    let expected_generation = serde_json::to_value(target.generation)
        .map_err(|error| format!("failed to encode expected Pause generation: {error}"))?;
    let identity_matches = value
        ...
        && value.get("kind").and_then(serde_json::Value::as_str) == Some("pause")
    ...
    if !identity_matches {
        return Err(format!(
            "durable Pause journal {} did not match the exact operation and generation",
            path.display()
        ));
    }
    let outcome = value
        .get("outcome")
        .ok_or_else(|| format!("durable Pause journal {} has no outcome", path.display()))?;
    match outcome.get("status").and_then(serde_json::Value::as_str) {
        Some("prepared") => Ok(PauseJournalStatus::Prepared),
        Some("succeeded") => {
            let response: ContainerRecord =
                serde_json::from_value(outcome.get("response").cloned().ok_or_else(|| {
                    format!(
                        "durable Pause journal {} has no container response",
                        path.display()
                    )
                })?)
                .map_err(|error| {
                    format!(
                        "failed to decode durable Pause response {}: {error}",
                        path.display()
                    )
                })?;
            ...
            {
                return Err(format!(
                    "durable Pause response {} changed its exact target",
                    path.display()
                ));
            }
