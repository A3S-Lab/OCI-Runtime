...
fn accepts_complete_shim_evidence() {
    let report = parse_shim_report(&valid_output("windows"), HostPlatform::Windows)
        .expect("valid Windows shim evidence");
    assert_eq!(report["guest_exit_code"], 0);

    let report = parse_shim_report(&valid_output("macos"), HostPlatform::Macos)
...
fn rejects_incomplete_or_truncated_shim_evidence() {
    ...
    assert!(parse_shim_report(&incomplete, HostPlatform::Windows).is_err());

    let mut truncated = valid_output("windows");
    truncated.truncated = true;
    assert!(parse_shim_report(&truncated, HostPlatform::Windows).is_err());
}

#[test]
fn rejects_a_shim_report_for_the_wrong_host() {
    assert!(parse_shim_report(&valid_output("windows"), HostPlatform::Macos).is_err());
}
