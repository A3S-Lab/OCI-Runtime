...
use super::{
    OciVmOperationReopenReplacementReport, OCI_VM_OPERATION_REOPEN_REPLACEMENT_EXEC_SCHEMA_VERSION,
    QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_exec(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version = OCI_VM_OPERATION_REOPEN_REPLACEMENT_EXEC_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::Exec;
        report.exec_terminal = Some(true);
        report
    }

    #[cfg(not(all(target_os = "macos", target_arch = "aarch64")))]
    pub(crate) fn unsupported_exec(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_exec(platform, requested_stage);
        ...
        report.reason = Some(
            "real utility-VM Exec reopen and owner replacement is implemented only for macOS aarch64/HVF"
                .to_string(),
        );
        report
    }

    pub(super) fn exec_evidence_succeeded(&self) -> bool {
        ...
        let setup_ids_are_distinct = self
            ...
            .is_some_and(|((exec, create), start)| {
                exec != create && exec != start && create != start
            });
        let expected_first_exec_pid = if response_delivered {
        ...
        matches!(self.platform, HostPlatform::Macos)
            ...
            && self.bundle_loaded
            && self.requested_operation == AgentOperation::Exec
            ...
            && self.exec_journal_prepared_before_reopen != response_delivered
            && self.exec_journal_succeeded_before_reopen == response_delivered
            && self.first_created_pid.is_some_and(|pid| pid > 0)
            && expected_first_exec_pid
            ...
            && self.replacement_rehydrated_exec_record == response_delivered
...
mod tests {
    ...
    #[test]
    fn exec_report_requires_live_process_rebind_identity_fencing_and_cleanup() {
        let mut report = OciVmOperationReopenReplacementReport::initial_exec(
            HostPlatform::Macos,
            AgentTransportOperationStage::HostBeforeRequestWrite,
        );
        report.status = CapabilityStatus::Available;
        report.bundle_loaded = true;
        report.qualification_operation_id = Some(OperationId::new("reopen-exec").expect("Exec ID"));
        report.setup_create_operation_id =
            Some(OperationId::new("reopen-exec-create").expect("Create ID"));
        report.setup_start_operation_id =
            Some(OperationId::new("reopen-exec-start").expect("Start ID"));
        report.container_id = Some(ContainerId::new("reopen-exec").expect("container ID"));
        ...
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.exec-host-before-request-write"
        ));
        ...
        for stage in AgentTransportOperationStage::ALL {
            ...
            stage_report.injected_point = Some(format!(
                "agent-v{AGENT_PROTOCOL_VERSION_MAX}.exec-{}",
                stage.as_str()
            ));
            if stage.is_guest() {
            ...
            if stage == AgentTransportOperationStage::GuestAfterResponseWrite {
                ...
                stage_report.first_exec_pid = Some(6_201);
                stage_report.replacement_rehydrated_exec_record = true;
