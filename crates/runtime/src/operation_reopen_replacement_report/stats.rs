...
use super::{
    OciVmOperationReopenReplacementReport, OCI_VM_OPERATION_REOPEN_REPLACEMENT_STATS_SCHEMA_VERSION,
};
...
impl OciVmOperationReopenReplacementReport {
    ...
    pub(crate) fn unsupported_stats(
        ...
        report.reason = Some(
            "the generic utility-VM Stats reopen command requires macOS aarch64/HVF; Linux uses the dedicated KVM Stats qualification"
                .to_string(),
        );
        report
    }
    ...
    pub(super) fn stats_evidence_succeeded(&self) -> bool {
        ...
        let update_resources_are_exact =
            crate::oci_smoke::utility_vm::lifecycle::resource_profile(self.platform)
    ...
    /// Unsupported targets must still compile the report model, but cannot
    /// claim utility-VM evidence whose qualification helpers are not built.
    #[cfg(not(any(
        all(target_os = "windows", target_arch = "x86_64"),
        all(target_os = "macos", target_arch = "aarch64"),
        all(
    ...
    pub(super) const fn stats_evidence_succeeded(&self) -> bool {
        false
    }
}
...
mod tests {
    ...
    fn stats_report_requires_fresh_updated_snapshot_rebinds_and_cleanup() {
        ...
        linux_report.update_resources = Some(
            crate::oci_smoke::utility_vm::lifecycle::resource_profile(HostPlatform::Linux)
                .expect("Linux resource profile"),
        );
