...
use super::{
    ...
    OCI_VM_OPERATION_REOPEN_REPLACEMENT_WRITE_STDIN_SCHEMA_VERSION, QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_write_stdin(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version =
            OCI_VM_OPERATION_REOPEN_REPLACEMENT_WRITE_STDIN_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::WriteStdin;
        report.exec_terminal = Some(false);
        report
    }

    #[cfg(not(all(target_os = "macos", target_arch = "aarch64")))]
    pub(crate) fn unsupported_write_stdin(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_write_stdin(platform, requested_stage);
        ...
        report.reason = Some(
            "the generic utility-VM WriteStdin reopen command requires macOS aarch64/HVF; Linux uses the dedicated KVM WriteStdin qualification"
                .to_string(),
        );
        report
    }

    pub(super) fn write_stdin_evidence_succeeded(&self) -> bool {
        ...
        let setup_ids_are_distinct = self
            ...
            .is_some_and(|(((write, create), start), exec)| {
                write != create
                    && write != start
                    && write != exec
                    && create != start
        ...
        matches!(self.platform, HostPlatform::Macos | HostPlatform::Linux)
            ...
            && self.bundle_loaded
            && self.requested_operation == AgentOperation::WriteStdin
            ...
            && self
                .write_stdin_data
                .as_deref()
                .is_some_and(|data| !data.is_empty() && data.ends_with(b"\n"))
            ...
            && self.write_stdin_journal_prepared_before_reopen != response_delivered
            && self.write_stdin_journal_succeeded_before_reopen == response_delivered
            ...
            && self.replacement_rehydrated_write_stdin == response_delivered
            ...
            && self.write_stdin_request_identity_reused
            ...
            && self.first_write_marker_verified
            && self.write_marker_reset_before_replacement
            && self.replacement_write_marker_verified
            ...
            && self.write_marker_absent_after_cleanup
...
mod tests {
    ...
    #[test]
    fn write_stdin_report_requires_exact_replay_identity_markers_and_cleanup() {
        ...
        report.qualification_operation_id =
            Some(OperationId::new("reopen-write-stdin").expect("WriteStdin ID"));
        report.setup_create_operation_id =
            Some(OperationId::new("reopen-write-stdin-create").expect("Create ID"));
        report.setup_start_operation_id =
            Some(OperationId::new("reopen-write-stdin-start").expect("Start ID"));
        report.setup_exec_operation_id =
            Some(OperationId::new("reopen-write-stdin-exec").expect("Exec ID"));
        report.container_id = Some(ContainerId::new("reopen-write-stdin").expect("container ID"));
        report.exec_process_id = Some(ProcessId::new("worker").expect("process ID"));
        report.write_stdin_data = Some(b"stdin\n".to_vec());
        report.negotiated_protocol = Some(AGENT_PROTOCOL_VERSION_MAX);
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.write-stdin-host-before-request-write"
        ));
        ...
        report.write_stdin_request_identity_reused = true;
        ...
        report.first_write_marker_verified = true;
        report.write_marker_reset_before_replacement = true;
        report.replacement_write_marker_verified = true;
        ...
        report.write_marker_absent_after_cleanup = true;
        ...
        for stage in AgentTransportOperationStage::ALL {
            ...
            stage_report.injected_point = Some(format!(
                "agent-v{AGENT_PROTOCOL_VERSION_MAX}.write-stdin-{}",
                stage.as_str()
            ));
            if stage.is_guest() {
            ...
            if stage == AgentTransportOperationStage::GuestAfterResponseWrite {
                ...
                stage_report.write_stdin_journal_prepared_before_reopen = false;
                stage_report.write_stdin_journal_succeeded_before_reopen = true;
                stage_report.replacement_rehydrated_write_stdin = true;
