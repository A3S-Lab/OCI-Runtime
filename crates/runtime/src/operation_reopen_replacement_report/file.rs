...
use super::{
    OciVmOperationReopenReplacementReport, OCI_VM_OPERATION_REOPEN_REPLACEMENT_FILE_SCHEMA_VERSION,
    QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_file(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version = OCI_VM_OPERATION_REOPEN_REPLACEMENT_FILE_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::File;
    ...
    pub(super) fn file_evidence_succeeded(&self) -> bool {
        ...
        matches!(self.platform, HostPlatform::Macos)
            ...
            && self.file_op == Some(FileOp::Upload)
            && self
                .file_path
                .as_deref()
                .is_some_and(|path| path.starts_with("/tmp/.a3s-oci-file-reopen-"))
            && self
                .file_data
                .as_deref()
            ...
            && self.replacement_rehydrated_file == response_delivered
            ...
            && self.replacement_file_response_verified
            && self.file_response_replayed
            && self.replacement_file_effect_verified
            ...
            && self.file_effect_absent_after_cleanup
...
mod tests {
    ...
    fn file_report_requires_session_replay_identity_effect_and_cleanup() {
        ...
        report.file_path = Some("/tmp/.a3s-oci-file-reopen-test.bin".to_string());
        report.file_data = Some("cmVwbGFjZW1lbnQ=".to_string());
        report.qualification_operation_id = Some(OperationId::new("file").expect("File ID"));
        report.setup_create_operation_id =
            Some(OperationId::new("file-create").expect("Create ID"));
        report.setup_start_operation_id = Some(OperationId::new("file-start").expect("Start ID"));
        report.container_id = Some(ContainerId::new("file").expect("container ID"));
        report.negotiated_protocol = Some(AGENT_PROTOCOL_VERSION_MAX);
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.file-host-before-request-write"
        ));
        ...
        delivered.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.file-guest-after-response-write"
        ));
