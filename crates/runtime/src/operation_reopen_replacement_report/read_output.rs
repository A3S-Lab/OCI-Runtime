...
use a3s_oci_sdk::OutputStream;

use super::{
    OciVmOperationReopenReplacementReport,
    OCI_VM_OPERATION_REOPEN_REPLACEMENT_READ_OUTPUT_SCHEMA_VERSION, QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_read_output(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version =
            OCI_VM_OPERATION_REOPEN_REPLACEMENT_READ_OUTPUT_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::ReadOutput;
        report.exec_terminal = Some(false);
        report
    }

    #[cfg(not(all(target_os = "macos", target_arch = "aarch64")))]
    pub(crate) fn unsupported_read_output(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_read_output(platform, requested_stage);
        ...
        report.reason = Some(
            "the generic utility-VM ReadOutput reopen command requires macOS aarch64/HVF; Linux uses the dedicated KVM ReadOutput qualification"
                .to_string(),
        );
        report
    }

    pub(super) fn read_output_evidence_succeeded(&self) -> bool {
        ...
        let setup_ids_are_distinct = self
            ...
            .is_some_and(|(((read, create), start), exec)| {
                read != create
                    && read != start
                    && read != exec
                    && create != start
        ...
        let expected_output_is_exact =
            self.expected_output_chunks
                .as_deref()
                .is_some_and(|chunks| {
                    chunks.len() == 1
                        && chunks[0].stream == OutputStream::Stdout
                        && !chunks[0].eof
                        && !chunks[0].data.is_empty()
                        && chunks[0].sequence
                            == u64::try_from(chunks[0].data.len()).unwrap_or_default()
                        && self.read_output_after_sequence == Some(0)
                        && self.read_output_max_bytes == u32::try_from(chunks[0].data.len()).ok()
                        && self.read_output_wait_timeout_ms == Some(5_000)
                });
        ...
        matches!(self.platform, HostPlatform::Macos | HostPlatform::Linux)
            ...
            && self.bundle_loaded
            && self.requested_operation == AgentOperation::ReadOutput
            ...
            && self.update_resources.is_none()
            && expected_output_is_exact
            ...
            && self.setup_update_operation_id.is_none()
            ...
            && self.first_output_chunks.is_some() == response_delivered
            && self.first_output_verified == response_delivered
            && (self.first_output_chunks.as_ref() == self.expected_output_chunks.as_ref())
                == response_delivered
            ...
            && self.first_stats_snapshot.is_none()
            ...
            && self.replacement_stats_snapshot.is_none()
            && self.replacement_output_chunks == self.expected_output_chunks
            && self.replacement_output_verified
            && self.output_response_rebound
            ...
            && self.read_output_request_identity_reused
...
mod tests {
    ...
    use a3s_oci_sdk::{
        ContainerId, ErrorCode, Generation, OperationId, OutputChunk, OutputStream, ProcessId,
    };
    use serde_json::json;
    ...
    #[test]
    fn read_output_report_requires_rebuilt_capture_identity_and_cleanup() {
        ...
        report.qualification_operation_id =
            Some(OperationId::new("reopen-read-output").expect("query nonce"));
        report.setup_create_operation_id =
            Some(OperationId::new("reopen-read-output-create").expect("Create ID"));
        report.setup_start_operation_id =
            Some(OperationId::new("reopen-read-output-start").expect("Start ID"));
        report.setup_exec_operation_id =
            Some(OperationId::new("reopen-read-output-exec").expect("Exec ID"));
        report.container_id = Some(ContainerId::new("reopen-read-output").expect("container ID"));
        report.exec_process_id = Some(ProcessId::new("reader").expect("process ID"));
        let expected = vec![OutputChunk {
            sequence: 4,
            stream: OutputStream::Stdout,
            data: b"read".to_vec(),
            eof: false,
        }];
        report.read_output_after_sequence = Some(0);
        report.read_output_max_bytes = Some(4);
        report.read_output_wait_timeout_ms = Some(5_000);
        report.expected_output_chunks = Some(expected.clone());
        report.negotiated_protocol = Some(AGENT_PROTOCOL_VERSION_MAX);
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.read-output-host-before-request-write"
        ));
        ...
        report.replacement_output_chunks = Some(expected.clone());
        report.replacement_output_verified = true;
        report.output_response_rebound = true;
        ...
        report.read_output_request_identity_reused = true;
        ...
        for stage in AgentTransportOperationStage::ALL {
            ...
            stage_report.injected_point = Some(format!(
                "agent-v{AGENT_PROTOCOL_VERSION_MAX}.read-output-{}",
                stage.as_str()
            ));
            if stage.is_guest() {
            ...
            if stage == AgentTransportOperationStage::GuestAfterResponseWrite {
                ...
                stage_report.first_output_chunks = Some(expected.clone());
                stage_report.first_output_verified = true;
            }
