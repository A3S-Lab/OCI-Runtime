...
use super::{
    ...
    OCI_VM_OPERATION_REOPEN_REPLACEMENT_RESIZE_SCHEMA_VERSION, QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_resize(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version =
            OCI_VM_OPERATION_REOPEN_REPLACEMENT_RESIZE_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::Resize;
        report.exec_terminal = Some(true);
        report.resize_size = Some(a3s_oci_sdk::TerminalSize {
            width: 120,
            height: 40,
        });
        report
    ...
    pub(crate) fn unsupported_resize(
        ...
        let mut report = Self::initial_resize(platform, requested_stage);
        ...
        report.reason = Some(
            "the generic utility-VM Resize reopen command requires macOS aarch64/HVF; Linux uses the dedicated KVM Resize qualification"
                .to_string(),
        );
        report
    }

    pub(super) fn resize_evidence_succeeded(&self) -> bool {
        ...
        let setup_ids_are_distinct = self
            ...
            .is_some_and(|(((resize, create), start), exec)| {
                resize != create
                    && resize != start
                    && resize != exec
                    && create != start
        ...
        matches!(self.platform, HostPlatform::Macos | HostPlatform::Linux)
            ...
            && self.resize_journal_prepared_before_reopen != response_delivered
            && self.resize_journal_succeeded_before_reopen == response_delivered
            ...
            && self.replacement_rehydrated_resize == response_delivered
            ...
            && self.resize_request_identity_reused
            ...
            && self.first_resize_marker_verified
            && self.resize_marker_reset_before_replacement
            && self.replacement_resize_marker_verified
            ...
            && self.resize_marker_absent_after_cleanup
...
mod tests {
    ...
    use a3s_oci_sdk::{ContainerId, ErrorCode, Generation, OperationId, ProcessId, TerminalSize};
    use serde_json::json;
    ...
    #[test]
    fn resize_report_requires_exact_replay_identity_markers_and_cleanup() {
        ...
        report.qualification_operation_id =
            Some(OperationId::new("reopen-resize").expect("Resize ID"));
        report.setup_create_operation_id =
            Some(OperationId::new("reopen-resize-create").expect("Create ID"));
        report.setup_start_operation_id =
            Some(OperationId::new("reopen-resize-start").expect("Start ID"));
        report.setup_exec_operation_id =
            Some(OperationId::new("reopen-resize-exec").expect("Exec ID"));
        report.container_id = Some(ContainerId::new("reopen-resize").expect("container ID"));
        report.exec_process_id = Some(ProcessId::new("worker").expect("process ID"));
        report.resize_size = Some(TerminalSize {
            width: 120,
            height: 40,
        });
        report.negotiated_protocol = Some(AGENT_PROTOCOL_VERSION_MAX);
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.resize-host-before-request-write"
        ));
        ...
        report.resize_request_identity_reused = true;
        ...
        report.first_resize_marker_verified = true;
        report.resize_marker_reset_before_replacement = true;
        report.replacement_resize_marker_verified = true;
        ...
        report.resize_marker_absent_after_cleanup = true;
        ...
        for stage in AgentTransportOperationStage::ALL {
            ...
            stage_report.injected_point = Some(format!(
                "agent-v{AGENT_PROTOCOL_VERSION_MAX}.resize-{}",
                stage.as_str()
            ));
            if stage.is_guest() {
            ...
            if stage == AgentTransportOperationStage::GuestAfterResponseWrite {
                ...
                stage_report.resize_journal_prepared_before_reopen = false;
                stage_report.resize_journal_succeeded_before_reopen = true;
                stage_report.replacement_rehydrated_resize = true;
