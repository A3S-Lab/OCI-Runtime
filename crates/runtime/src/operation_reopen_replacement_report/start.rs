...
use super::{
    ...
    OCI_VM_OPERATION_REOPEN_REPLACEMENT_START_SCHEMA_VERSION, QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_start(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version =
            OCI_VM_OPERATION_REOPEN_REPLACEMENT_START_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::Start;
        report
    }

    #[cfg(not(all(target_os = "macos", target_arch = "aarch64")))]
    pub(crate) fn unsupported_start(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_start(platform, requested_stage);
        ...
        report.reason = Some(
            "real utility-VM Start reopen and owner replacement is implemented only for macOS aarch64/HVF"
                .to_string(),
        );
        report
    }

    pub(super) fn start_evidence_succeeded(&self) -> bool {
        ...
        matches!(self.platform, HostPlatform::Macos)
            ...
            && self.bundle_loaded
            && self.requested_operation == AgentOperation::Start
            ...
            && self.durable_created_retained != response_delivered
...
mod tests {
    ...
    #[test]
    fn start_report_requires_all_nine_exact_handoffs_and_running_rebind() {
        let mut report = OciVmOperationReopenReplacementReport::initial_start(
            HostPlatform::Macos,
        ...
        report.qualification_operation_id =
            Some(OperationId::new("reopen-start").expect("Start ID"));
        report.setup_create_operation_id =
            Some(OperationId::new("reopen-start-create").expect("Create ID"));
        report.container_id = Some(ContainerId::new("reopen-start").expect("container ID"));
        report.negotiated_protocol = Some(AGENT_PROTOCOL_VERSION_MAX);
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.start-host-before-request-write"
        ));
        ...
        for stage in AgentTransportOperationStage::ALL {
            ...
            stage_report.injected_point = Some(format!(
                "agent-v{AGENT_PROTOCOL_VERSION_MAX}.start-{}",
                stage.as_str()
            ));
            if stage.is_guest() {
            ...
            if stage == AgentTransportOperationStage::GuestAfterResponseWrite {
                ...
                stage_report.durable_created_retained = false;
                stage_report.durable_running_retained = true;
                stage_report.replacement_rehydrated_running_record = true;
        ...
        for incomplete in [
            ...
            OciVmOperationReopenReplacementReport {
                replacement_operation_dispatches: 2,
                ..report.clone()
            },
        ] {
