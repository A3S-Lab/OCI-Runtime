...
impl OciVmOperationReopenReplacementReport {
    ...
    pub(crate) fn unsupported_filesystem(
        ...
        report.reason = Some(
            "real utility-VM Filesystem reopen and owner replacement requires macOS aarch64/HVF or Linux KVM"
                .to_string(),
        );
        report
    }
    ...
    pub(super) fn filesystem_evidence_succeeded(&self) -> bool {
        ...
        matches!(self.platform, HostPlatform::Macos | HostPlatform::Linux)
            ...
            && !self.first_filesystem_response_verified
...
mod tests {
    ...
    #[test]
    fn filesystem_report_requires_durable_replay_identity_effect_and_cleanup() {
