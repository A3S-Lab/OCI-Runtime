...
use super::{
    ...
    OCI_VM_OPERATION_REOPEN_REPLACEMENT_PAUSE_SCHEMA_VERSION, QUALIFICATION_FAULT_OPERATION,
};

impl OciVmOperationReopenReplacementReport {
    pub(crate) fn initial_pause(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_state(platform, requested_stage);
        report.schema_version =
            OCI_VM_OPERATION_REOPEN_REPLACEMENT_PAUSE_SCHEMA_VERSION.to_string();
        report.requested_operation = AgentOperation::Pause;
        report
    }

    #[cfg(not(all(target_os = "macos", target_arch = "aarch64")))]
    pub(crate) fn unsupported_pause(
        platform: HostPlatform,
        requested_stage: AgentTransportOperationStage,
    ) -> Self {
        let mut report = Self::initial_pause(platform, requested_stage);
        ...
        report.reason = Some(
            "the generic utility-VM Pause reopen command is implemented for macOS aarch64/HVF; Linux KVM uses its dedicated qualification entry"
                .to_string(),
        );
        report
    }

    pub(super) fn pause_evidence_succeeded(&self) -> bool {
        ...
        let setup_ids_are_distinct = self
            ...
            .is_some_and(|((pause, create), start)| {
                pause != create && pause != start && create != start
            });
        ...
        matches!(self.platform, HostPlatform::Macos | HostPlatform::Linux)
            ...
            && self.bundle_loaded
            && self.requested_operation == AgentOperation::Pause
            ...
            && self.pause_journal_prepared_before_reopen != response_delivered
            && self.pause_journal_succeeded_before_reopen == response_delivered
            ...
            && self.replacement_rehydrated_paused_record == response_delivered
...
mod tests {
    ...
    #[test]
    fn pause_report_requires_exact_freezer_recovery_rebinds_and_cleanup() {
        let mut report = OciVmOperationReopenReplacementReport::initial_pause(
            HostPlatform::Macos,
        ...
        report.qualification_operation_id =
            Some(OperationId::new("reopen-pause").expect("Pause ID"));
        report.setup_create_operation_id =
            Some(OperationId::new("reopen-pause-create").expect("Create ID"));
        report.setup_start_operation_id =
            Some(OperationId::new("reopen-pause-start").expect("Start ID"));
        report.container_id = Some(ContainerId::new("reopen-pause").expect("container ID"));
        report.negotiated_protocol = Some(AGENT_PROTOCOL_VERSION_MAX);
        report.injected_point = Some(format!(
            "agent-v{AGENT_PROTOCOL_VERSION_MAX}.pause-host-before-request-write"
        ));
        ...
        for stage in AgentTransportOperationStage::ALL {
            ...
            stage_report.injected_point = Some(format!(
                "agent-v{AGENT_PROTOCOL_VERSION_MAX}.pause-{}",
                stage.as_str()
            ));
            if stage.is_guest() {
            ...
            if stage == AgentTransportOperationStage::GuestAfterResponseWrite {
                ...
                stage_report.durable_paused_retained = true;
                stage_report.pause_journal_prepared_before_reopen = false;
                stage_report.pause_journal_succeeded_before_reopen = true;
                stage_report.replacement_rehydrated_paused_record = true;
