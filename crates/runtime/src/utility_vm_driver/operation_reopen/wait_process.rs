...
use crate::operation_reopen_replacement_report::wait_process::WAIT_PROCESS_SIGNAL;
...
use flow::support::waitable_exec_process;

/// Exact inputs for one real Linux KVM non-init WaitProcess interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmWaitProcessReopenConfig {
    pub shim: PathBuf,
...
pub(super) struct Qualification {
    ...
    qualification_operation_id: a3s_oci_sdk::OperationId,
...
/// Rebuild and terminate one Exec before replaying an interrupted WaitProcess.
///
/// Create, Start, Exec, setup SignalProcess, WaitProcess, exit-cache recovery,
/// stale-generation fencing, and cleanup all use the production immutable-image,
...
pub async fn linux_kvm_wait_process_reopen_replacement(
    config: LinuxKvmWaitProcessReopenConfig,
    ...
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(
            report,
            "Linux KVM WaitProcess reopen qualification requires Linux",
        );
    }
    ...
    let container_id =
        match a3s_oci_sdk::ContainerId::new(format!("kvm-wait-process-reopen-{nonce}")) {
            Ok(id) => id,
            Err(error) => {
                return failed(report, format!("failed to construct container ID: {error}"));
            }
        };
    let create_operation_id = match operation_id(&format!("kvm-wait-process-reopen-{nonce}-create"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-wait-process-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("kvm-wait-process-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let signal_operation_id = match operation_id(&format!("kvm-wait-process-reopen-{nonce}-signal"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let qualification_operation_id =
        match operation_id(&format!("kvm-wait-process-reopen-{nonce}-qualification")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let delete_operation_id = match operation_id(&format!("kvm-wait-process-reopen-{nonce}-delete"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    let source_rootfs = match source_init_marker.parent() {
        Some(rootfs) => rootfs,
        None => return failed(report, "KVM WaitProcess init marker has no rootfs parent"),
    };
    ...
    for (label, marker) in [("init", &source_init_marker), ("Exec", &source_exec_marker)] {
        if marker.exists() {
            return failed(
                report,
                format!(
                    "refusing to use a KVM WaitProcess qualification bundle with an existing {label} marker: {}",
                    marker.display()
                ),
            );
        }
    ...
    let (process_id, process, io) = match waitable_exec_process(&nonce) {
        Ok(process) => process,
        Err(reason) => return failed(report, reason),
    };
    let signal = match Signal::new(WAIT_PROCESS_SIGNAL) {
        Ok(signal) => signal,
        Err(error) => {
            return failed(
                report,
                format!("failed to construct WaitProcess setup signal: {error}"),
            );
        }
    };
    report.bundle_loaded = true;
    report.qualification_operation_id = Some(qualification_operation_id.clone());
    ...
    report.setup_signal_process_operation_id = Some(signal_operation_id.clone());
    ...
    let qualification = Qualification {
        ...
        exec_marker_contents: format!("a3s-oci-wait-process-{nonce}\n").into_bytes(),
        stage: config.stage,
    };
    ...
    let first_console = prepared
        .console_directory
        .join("wait-process-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("wait-process-reopen-replacement.log");
    ...
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM WaitProcess reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM WaitProcess reopen modified the private bootstrap root {}",
