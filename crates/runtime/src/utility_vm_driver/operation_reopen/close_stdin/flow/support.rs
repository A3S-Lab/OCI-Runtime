...
use a3s_oci_sdk::{
    CloseStdinRequest, ContainerTarget, DeleteRequest, Error, ErrorCode, ExecRequest, IoMode,
    OperationId, ProcessId, ProcessIo, ProcessTarget, StartRequest,
};
...
pub(super) use crate::operation_journal_evidence::EmptyOperationJournalStatus as CloseStdinJournalStatus;
...
pub(in crate::utility_vm_driver::operation_reopen::close_stdin) const CLOSE_MARKER_NAME: &str =
    ".a3s-oci-close-stdin-reopen-smoke";
...
pub(in crate::utility_vm_driver::operation_reopen::close_stdin) fn stdin_exec_process(
    ...
    {
        return Err("KVM CloseStdin qualification nonce contains unsafe shell characters".into());
    }
    ...
    .map_err(|error| format!("failed to construct stdin-close Exec process: {error}"))?;
    let io = ProcessIo {
        stdin: IoMode::Pipe,
        stdout: IoMode::Null,
        stderr: IoMode::Null,
...
pub(super) fn process_marker(
    ...
    init_marker
        ...
        .ok_or_else(|| format!("KVM CloseStdin {label} marker has no rootfs parent"))
}
...
pub(super) async fn verify_first_close_marker(
    ...
    if !dispatch_may_have_reached(stage) && marker_exists {
        return Err(format!(
            "{} produced a KVM CloseStdin marker before Guest dispatch",
            stage.as_str()
        ));
    }
    if !marker_exists {
    ...
    let contents = tokio::fs::read(marker).await.map_err(|error| {
        format!(
            "failed to read first-owner KVM CloseStdin marker {}: {error}",
            marker.display()
        )
    })?;
    match exact_marker_state(&contents, expected) {
        ...
        ExactMarkerState::Mismatch => {
            Err("first-owner KVM CloseStdin produced unexpected marker contents".to_string())
        }
    }
}
...
) -> Result<CloseStdinJournalStatus, String> {
...
pub(super) fn record_interruption(
    ...
    if error.code == ErrorCode::Unavailable && error.retryable && expected_operation {
        ...
        Err(format!(
            "first KVM owner returned an unexpected CloseStdin transport error at {}: {error}",
            stage.as_str()
        ))
    }
}
...
#[cfg(test)]
mod tests {
    use super::{stdin_exec_process, CLOSE_MARKER_NAME};
    ...
    fn stdin_exec_is_nonce_bound_pipe_backed_workload() {
        let (process_id, process, io) =
            stdin_exec_process("close-123").expect("stdin Exec payload");
        assert_eq!(process_id.as_str(), "close-worker-close-123");
        ...
        assert!(process
            ...
            .is_some_and(|command| {
                command.contains("while IFS= read -r input")
                    && command.contains(CLOSE_MARKER_NAME)
