...
use a3s_oci_sdk::{
    CreateRequest, IsolationRequest, OciBundle, OperationContext, ProcessIo, TerminalSize,
};
...
use flow::support::{terminal_exec_process, RESIZE_MARKER_NAME};

/// Exact inputs for one real Linux KVM terminal Resize interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmResizeReopenConfig {
    pub shim: PathBuf,
...
pub(super) struct Qualification {
    ...
    resize_operation_id: a3s_oci_sdk::OperationId,
    ...
    process_id: a3s_oci_sdk::ProcessId,
...
/// Rebuild or replay one terminal Resize in a distinct real KVM owner.
///
/// Create, Start, Exec, Resize, process-record rebinding, stale-generation
...
pub async fn linux_kvm_resize_reopen_replacement(
    config: LinuxKvmResizeReopenConfig,
) -> OciVmOperationReopenReplacementReport {
    let mut report = OciVmOperationReopenReplacementReport::initial_resize(
    ...
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(
            report,
            "Linux KVM Resize reopen qualification requires Linux",
        );
    }
    ...
    let container_id = match a3s_oci_sdk::ContainerId::new(format!("kvm-resize-reopen-{nonce}")) {
        Ok(id) => id,
        Err(error) => {
            return failed(report, format!("failed to construct container ID: {error}"));
        }
    };
    let create_operation_id = match operation_id(&format!("kvm-resize-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-resize-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("kvm-resize-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let resize_operation_id = match operation_id(&format!("kvm-resize-reopen-{nonce}-resize")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("kvm-resize-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id =
        match operation_id(&format!("kvm-resize-reopen-{nonce}-stale-guest")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let stale_host_operation_id =
        match operation_id(&format!("kvm-resize-reopen-{nonce}-stale-host")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    ...
    let source_rootfs = match source_init_marker.parent() {
        Some(rootfs) => rootfs,
        None => return failed(report, "KVM Resize init marker has no rootfs parent"),
    };
    let source_exec_marker = source_rootfs.join(EXEC_MARKER_NAME);
    let source_resize_marker = source_rootfs.join(RESIZE_MARKER_NAME);
    for (label, marker) in [
        ("init", &source_init_marker),
        ("Exec", &source_exec_marker),
        ("Resize", &source_resize_marker),
    ] {
        if marker.exists() {
            return failed(
                report,
                format!(
                    "refusing to use a KVM Resize qualification bundle with an existing {label} marker: {}",
                    marker.display()
                ),
            );
        }
    ...
    let (process_id, process, io) = match terminal_exec_process(&nonce) {
        Ok(process) => process,
        Err(reason) => return failed(report, reason),
    };
    ...
    report.qualification_operation_id = Some(resize_operation_id.clone());
    ...
    let qualification = Qualification {
        ...
        resize_marker_contents: format!("a3s-oci-resize-{nonce}:40 120\n").into_bytes(),
        stage: config.stage,
    };
    ...
    let first_console = prepared.console_directory.join("resize-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("resize-reopen-replacement.log");
    ...
    report.resize_marker_absent_after_cleanup =
        report.resize_marker_absent_after_cleanup && !source_resize_marker.exists();
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM Resize reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM Resize reopen modified the private bootstrap root {}",
