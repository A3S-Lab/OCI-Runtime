...
use a3s_oci_agent_protocol::AgentResizeRequest;
use a3s_oci_core::{DriverKind, IsolationClass};
use a3s_oci_sdk::oci_spec::runtime::ContainerState;
use a3s_oci_sdk::{
    ErrorCode, ListRequest, OciRuntimeService, OperationContext, ProcessTarget, ResizeRequest,
};
...
use super::support::{
    append_failure, durable_exec_process, path_absent, resize_journal_status, FirstOwnerOutcome,
    ResizeJournalStatus,
};
...
pub(super) async fn run(
    ...
    let recovery_resize = first.response_delivered.then(|| first.resize.clone());
    ...
    let driver = Arc::new(QualificationKvmOperationDriver::with_resize_recovery(
        prepared,
    ...
    let service =
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                report.resize_marker_absent_after_cleanup =
                    path_absent(&first.resize_marker).await.unwrap_or(false);
    ...
    if report.replacement_rehydrated_resize != first.response_delivered {
        append_failure(
            &mut failure,
            "replacement KVM Resize replay did not match the durable journal outcome",
        );
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
                ...
                || *record.state.pid() != report.replacement_created_pid
            {
                append_failure(
                    &mut failure,
                    format!(
                        "replacement KVM Resize recovery retained invalid {} record with PID {:?}",
                        record.state.status(),
                        record.state.pid()
                    ),
                );
        ...
        Ok(records) => append_failure(
            &mut failure,
            format!(
                "replacement KVM Resize recovery retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut failure,
            format!("failed to inspect recovered KVM Resize record: {error}"),
        ),
    }
    ...
    if !first.response_delivered {
        match path_absent(&first.resize_marker).await {
            Ok(true) => {}
            Ok(false) => append_failure(
                &mut failure,
                "replacement KVM recovery dispatched an uncommitted Resize before API retry",
            ),
            Err(reason) => append_failure(&mut failure, reason),
        }
    }
    let calls_before_resize = driver.resize_calls();
    match timeout(QUALIFICATION_TIMEOUT, service.resize(first.resize.clone())).await {
        Ok(Ok(())) => {
            report.operation_completed_after_reopen = true;
            report.generation_after_reopen = first.resize.process.container.generation;
            ...
            if !report.same_generation_reused {
                append_failure(
                    &mut failure,
                    "replacement KVM Resize did not retain the durable generation",
                );
            }
        }
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("replacement KVM Resize failed: {error}"),
        ),
        Err(_) => append_failure(&mut failure, "replacement KVM Resize timed out"),
    }
    report.operation_replayed_without_driver_dispatch =
        driver.resize_calls() == calls_before_resize;
    match wait_for_exact_marker(
        &first.resize_marker,
        &qualification.resize_marker_contents,
        "replacement KVM Resize",
    )
    .await
    {
        Ok(()) => report.replacement_resize_marker_verified = true,
        Err(reason) => append_failure(&mut failure, reason),
    }
    match resize_journal_status(
        state_root,
        &first.resize.context.operation_id,
        &first.resize.process,
    )
    .await
    {
        Ok(ResizeJournalStatus::SucceededEmpty) => {
            report.replacement_response_matches_durable_record = true;
        }
        Ok(ResizeJournalStatus::Prepared) => append_failure(
            &mut failure,
            "replacement KVM Resize journal remained prepared",
        ),
        Err(reason) => append_failure(&mut failure, reason),
    }
    if let Some(process) = replacement_exec.as_ref() {
        match durable_exec_process(state_root, &exact_process).await {
            Ok(durable) => {
                ...
                if !report.exec_response_rebound {
                    append_failure(
                        &mut failure,
                        "completed KVM Resize did not retain the rebound Exec record",
                    );
                }
            }
            Err(reason) => append_failure(&mut failure, reason),
    ...
    let calls_before_replay = driver.resize_calls();
    match timeout(QUALIFICATION_TIMEOUT, service.resize(first.resize.clone())).await {
        Ok(Ok(())) if driver.resize_calls() == calls_before_replay => {}
        Ok(Ok(())) => append_failure(
            &mut failure,
            "later KVM Resize replay reached the replacement driver",
        ),
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("later KVM Resize replay failed: {error}"),
        ),
        Err(_) => append_failure(&mut failure, "later KVM Resize replay timed out"),
    }
    report.replacement_operation_dispatches = driver.resize_calls();
    if report.operation_replayed_without_driver_dispatch != first.response_delivered {
        append_failure(
            &mut failure,
            "replacement KVM Resize dispatch did not match the durable journal outcome",
        );
    }
    if report.replacement_operation_dispatches != 1 {
        append_failure(
            &mut failure,
            format!(
                "replacement KVM driver recorded {} Resize dispatches instead of one",
    ...
    match driver.resize_identity() {
        Ok(identity) => {
            report.resize_request_identity_reused = identity == first.resize_identity;
            report.same_operation_id_reused = report.resize_request_identity_reused
                && identity.context.operation_id == first.resize.context.operation_id
                && identity.target == exact_process
                && identity.size == first.resize.size;
            if !report.resize_request_identity_reused || !report.same_operation_id_reused {
                append_failure(
                    &mut failure,
                    "replacement KVM Resize changed its operation, target, or size identity",
                );
            }
        }
        Err(reason) => append_failure(&mut failure, reason),
    ...
    let mut changed_host = first.resize.clone();
    ...
    match service.resize(changed_host).await {
        Err(error)
            if error.code == ErrorCode::FailedPrecondition
                && driver.resize_calls() == calls_before_changed_host =>
        {
            report.host_changed_request_rejected = true;
        }
        Err(error) => append_failure(
            &mut failure,
            format!("reopened KVM Host returned the wrong changed Resize error: {error}"),
        ),
        Ok(()) => append_failure(&mut failure, "reopened KVM Host accepted changed Resize"),
    }
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("replacement KVM Guest returned the wrong stale Resize error: {error}"),
        ),
        Ok(Ok(())) => append_failure(&mut failure, "replacement KVM Guest accepted stale Resize"),
        Err(_) => append_failure(
            &mut failure,
            "replacement KVM Guest stale Resize check timed out",
        ),
    }
    let calls_before_stale_host = driver.resize_calls();
    match service
        ...
        .await
    {
        Err(error)
            if error.code == ErrorCode::Conflict
                && driver.resize_calls() == calls_before_stale_host =>
        {
            report.host_stale_generation_rejected = true;
        }
        Err(error) => append_failure(
            &mut failure,
            format!("reopened KVM Host returned the wrong stale Resize error: {error}"),
        ),
        Ok(()) => append_failure(&mut failure, "reopened KVM Host accepted stale Resize"),
    }
    ...
    match path_absent(&first.resize_marker).await {
        Ok(absent) => report.resize_marker_absent_after_cleanup = absent,
        Err(reason) => append_failure(&mut failure, reason),
    }
        ...
        || !report.resize_marker_absent_after_cleanup
    {
        append_failure(
            &mut failure,
            "replacement KVM Resize marker remained after cleanup",
        );
    }
    if !report.replacement_guest_runtime_clean {
        append_failure(
            &mut failure,
            "replacement KVM Resize owner left its runtime share",
        );
    }
    if !report.owners_distinct {
        append_failure(
            &mut failure,
            "first and replacement KVM Resize owner identities were not distinct",
        );
    }
