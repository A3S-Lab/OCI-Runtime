...
use a3s_oci_sdk::{
    ...
    OperationContext, ResizeRequest, StartRequest,
};
...
use super::support::{
    ...
    reset_marker, resize_journal_status, runtime_marker, verify_first_resize_marker,
    FirstOwnerOutcome, ResizeJournalStatus, RESIZE_MARKER_NAME,
};
use crate::agent_session::UtilityVmSessionQualification;
use crate::driver::{DriverExecRequest, DriverResizeRequest, RuntimeDriver};
...
pub(super) async fn run(
    ...
    let guest_qualification = if qualification.stage.is_guest() {
        Some(
            AgentTransportQualificationRequest::new(
                qualification.resize_operation_id.clone(),
                AgentOperation::Resize,
                qualification.stage,
            )
            .map_err(|error| {
                format!("failed to construct Guest KVM Resize qualification: {error}")
            })?,
        )
    } else {
        None
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            ...
            return setup_failure(
                ...
                report,
                format!(
                    "KVM Resize setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            ...
            return setup_failure(
                &driver,
                report,
                format!("KVM Resize setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return setup_failure(
                &driver,
                report,
                "KVM Resize setup Create timed out".to_string(),
            )
            .await;
        }
    };
    ...
    let resize_marker = match process_marker(&init_marker, RESIZE_MARKER_NAME, "Resize") {
        Ok(marker) => marker,
        Err(reason) => {
            drop(service);
            return active_failure(&driver, &target, report, reason).await;
        ...
        match path_absent(marker).await {
            ...
            Ok(false) => {
                ...
                return active_failure(
                    ...
                    report,
                    format!(
                        "KVM Resize {label} marker existed before setup Start: {}",
                        marker.display()
                    ),
                )
                .await;
    ...
    let started = match timeout(QUALIFICATION_TIMEOUT, service.start(start.clone())).await {
        ...
        Ok(Ok(record)) => {
            ...
            return active_failure(
                ...
                report,
                format!(
                    "KVM Resize setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM Resize setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM Resize setup Start timed out".to_string(),
            )
            .await;
        }
    };
    ...
    let first_exec = match timeout(QUALIFICATION_TIMEOUT, service.exec(exec.clone())).await {
        ...
        Ok(Ok(process)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM Resize setup Exec returned invalid process {process:?}"),
            )
            .await;
        }
        Ok(Err(error)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM Resize setup Exec failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM Resize setup Exec timed out".to_string(),
            )
            .await;
        }
    };
    ...
    match (
        ...
        (Ok(ProcessOperationJournalStatus::Succeeded(_)), Ok(_)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM Resize setup Exec journal changed its durable process".to_string(),
            )
            .await;
        }
        (Ok(ProcessOperationJournalStatus::Prepared), _) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM Resize setup Exec journal remained prepared".to_string(),
            )
            .await;
        }
    ...
    let resize = ResizeRequest {
        context: OperationContext::new(qualification.resize_operation_id.clone()),
    ...
    match timeout(QUALIFICATION_TIMEOUT, service.resize(resize.clone())).await {
        Ok(Err(error)) => {
        ...
        Ok(Ok(())) => append_failure(
            &mut failure,
            "first KVM Resize unexpectedly completed before owner replacement",
        ),
        Err(_) => append_failure(&mut failure, "first KVM Resize timed out"),
    }
    report.first_operation_dispatches = driver.resize_calls();
    ...
    match verify_first_resize_marker(
        &resize_marker,
    ...
    .await
    {
        Ok(()) => report.first_resize_marker_verified = true,
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut failure,
                    "interrupted KVM Resize did not retain the exact durable running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut failure,
            format!(
                "interrupted KVM Resize retained {} durable records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut failure,
            format!("failed to inspect durable state after interrupted KVM Resize: {error}"),
        ),
    }
    match resize_journal_status(state_root, &resize.context.operation_id, &resize.process).await {
        Ok(ResizeJournalStatus::Prepared) => {
            report.resize_journal_prepared_before_reopen = true;
            if response_delivered {
                append_failure(
                    &mut failure,
                    "delivered first KVM Resize response left its journal prepared",
                );
            }
        }
        Ok(ResizeJournalStatus::SucceededEmpty) => {
            report.resize_journal_succeeded_before_reopen = true;
            report.first_response_matches_durable_record = response_delivered;
            if !response_delivered {
                append_failure(
                    &mut failure,
                    "KVM Resize journal committed before its response boundary",
                );
            }
        }
        Err(reason) => append_failure(&mut failure, reason),
    ...
    let expected_driver_resize = DriverResizeRequest {
        context: resize.context.clone(),
        target: exact_process,
        size: resize.size,
    };
    let start_identity = driver.start_identity();
    let exec_identity = driver.exec_identity();
    let resize_identity = driver.resize_identity();
        ...
        match read_guest_qualification_evidence(first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut failure,
                        "Guest KVM Resize evidence did not match the exact qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut failure, reason),
    ...
    for (marker, field) in [
        ...
        (
            &resize_marker,
            &mut report.resize_marker_reset_before_replacement,
        ),
    ] {
    ...
    if !report.first_guest_runtime_clean {
        append_failure(
            &mut failure,
            "first KVM Resize owner left Guest Agent runtime state",
        );
    }
    for (label, actual) in [
        ("Start", driver.start_calls()),
        ("Exec", driver.exec_calls()),
        ("Resize", report.first_operation_dispatches),
    ] {
        if actual != 1 {
            append_failure(
                &mut failure,
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut failure,
            format!(
                "selected KVM Resize point crossed {} times instead of once",
    ...
    let resize_identity = match resize_identity {
        Ok(identity) => identity,
        Err(reason) => {
            append_failure(&mut failure, reason);
            expected_driver_resize
        }
    };
