...
/// Exact inputs for one real Linux KVM init Wait interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmWaitReopenConfig {
    pub shim: PathBuf,
...
/// Rebuild or replay one init Wait in a distinct KVM owner.
///
/// Create, Start, setup Kill, Wait, marker verification, stale-generation
/// fencing, and cleanup all use the production immutable-image,
/// exact-generation share, handoff, and authenticated Guest Agent path. This
/// remains qualification-only and does not register the probe-only KVM
/// candidate for workload selection.
pub async fn linux_kvm_wait_reopen_replacement(
    config: LinuxKvmWaitReopenConfig,
) -> OciVmOperationReopenReplacementReport {
    let mut report =
        OciVmOperationReopenReplacementReport::initial_wait(HostPlatform::current(), config.stage);
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(report, "Linux KVM Wait reopen qualification requires Linux");
    }
    ...
    let container_id = match a3s_oci_sdk::ContainerId::new(format!("kvm-wait-reopen-{nonce}")) {
        Ok(id) => id,
        Err(error) => return failed(report, format!("failed to construct container ID: {error}")),
    };
    let create_operation_id = match operation_id(&format!("kvm-wait-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-wait-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let kill_operation_id = match operation_id(&format!("kvm-wait-reopen-{nonce}-kill")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let qualification_operation_id =
        match operation_id(&format!("kvm-wait-reopen-{nonce}-qualification")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let delete_operation_id = match operation_id(&format!("kvm-wait-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    if source_marker.exists() {
        return failed(
            report,
            format!(
                "refusing to use a KVM Wait qualification bundle with an existing marker: {}",
    ...
    let first_console = prepared.console_directory.join("wait-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("wait-reopen-replacement.log");
    ...
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM Wait reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM Wait reopen modified the private bootstrap root {}",
