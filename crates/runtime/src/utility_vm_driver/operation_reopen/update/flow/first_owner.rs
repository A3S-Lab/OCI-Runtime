...
use super::support::{
    append_failure, identity_or_expected, path_absent, record_interruption, reset_marker,
    runtime_marker, update_journal_status, wait_for_exact_marker, FirstOwnerOutcome,
    UpdateJournalStatus,
};
...
pub(super) async fn run(
    ...
    let guest_qualification = if qualification.stage.is_guest() {
        Some(
            AgentTransportQualificationRequest::new(
                qualification.update_operation_id.clone(),
                AgentOperation::Update,
                qualification.stage,
            )
            .map_err(|error| {
                format!("failed to construct Guest KVM Update qualification: {error}")
            })?,
        )
    } else {
        None
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            ...
            return setup_failure(
                ...
                report,
                format!(
                    "KVM Update setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            ...
            return setup_failure(
                &driver,
                report,
                format!("KVM Update setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return setup_failure(
                &driver,
                report,
                "KVM Update setup Create timed out".to_string(),
            )
            .await;
        }
    };
    ...
    match path_absent(&init_marker).await {
        ...
        Ok(false) => {
            ...
            return active_failure(
                ...
                report,
                format!(
                    "KVM Update init marker existed before setup Start: {}",
                    init_marker.display()
                ),
            )
            .await;
    ...
    let started = match timeout(QUALIFICATION_TIMEOUT, service.start(start.clone())).await {
        ...
        Ok(Ok(record)) => {
            ...
            return active_failure(
                ...
                report,
                format!(
                    "KVM Update setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM Update setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM Update setup Start timed out".to_string(),
            )
            .await;
        }
    };
    ...
    match timeout(QUALIFICATION_TIMEOUT, service.update(update.clone())).await {
        Ok(Err(error)) => {
        ...
        Ok(Ok(record)) => append_failure(
            &mut failure,
            format!("first KVM Update unexpectedly completed before owner replacement: {record:?}"),
        ),
        Err(_) => append_failure(&mut failure, "first KVM Update timed out"),
    }
    ...
    let durable = match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut failure,
                    "interrupted KVM Update did not retain the exact durable running record",
                );
            }
            Some(record)
        }
        Ok(records) => {
            append_failure(
                &mut failure,
                format!(
                    "interrupted KVM Update retained {} durable records instead of one",
                    records.len()
                ),
            );
            None
        ...
        Err(error) => {
            append_failure(
                &mut failure,
                format!("failed to inspect durable state after KVM Update: {error}"),
            );
            None
        }
    };
    match update_journal_status(state_root, &update.context.operation_id, &target).await {
        Ok(UpdateJournalStatus::Prepared) => {
            ...
            if response_delivered {
                append_failure(
                    &mut failure,
                    "delivered KVM Update response left its journal prepared",
                );
            }
        }
        Ok(UpdateJournalStatus::Succeeded(journal)) => {
            ...
            report.first_response_matches_durable_record =
                response_delivered && durable.as_ref() == Some(journal.as_ref());
            if !report.first_response_matches_durable_record {
                append_failure(
                    &mut failure,
                    "completed KVM Update journal did not match its durable record",
                );
            }
            if !response_delivered {
                append_failure(
                    &mut failure,
                    "KVM Update journal committed before its response boundary",
                );
            }
        }
        ...
        match read_guest_qualification_evidence(first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut failure,
                        "Guest KVM Update evidence did not match the exact qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut failure, reason),
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut failure,
            format!(
                "selected KVM Update point crossed {} times instead of once",
