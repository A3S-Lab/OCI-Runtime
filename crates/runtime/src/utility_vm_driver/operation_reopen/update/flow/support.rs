...
pub(super) fn record_interruption(
    ...
    if error.code == ErrorCode::Unavailable && error.retryable && expected_operation {
        ...
        Err(format!(
            "first KVM owner returned an unexpected Update transport error at {}: {error}",
            stage.as_str()
        ))
    }
}
...
pub(super) fn changed_resources(
    original: &a3s_oci_sdk::oci_spec::runtime::LinuxResources,
) -> Result<a3s_oci_sdk::oci_spec::runtime::LinuxResources, String> {
