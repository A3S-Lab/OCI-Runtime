...
/// Exact inputs for one real Linux KVM Start interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmStartReopenConfig {
    pub shim: PathBuf,
...
/// Rebuild one created or running container in a distinct KVM owner and replay Start.
///
/// The setup Create, recovery Start, and workload marker all traverse the
/// production immutable image, exact-generation runtime share, bundle handoff,
/// and authenticated Guest Agent path. This remains qualification-only and
/// does not register the probe-only KVM candidate for workload selection.
pub async fn linux_kvm_start_reopen_replacement(
    config: LinuxKvmStartReopenConfig,
) -> OciVmOperationReopenReplacementReport {
    let mut report =
        OciVmOperationReopenReplacementReport::initial_start(HostPlatform::current(), config.stage);
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(
            report,
            "Linux KVM Start reopen qualification requires Linux",
        );
    }
    ...
    let container_id = match a3s_oci_sdk::ContainerId::new(format!("kvm-start-reopen-{nonce}")) {
        Ok(id) => id,
        Err(error) => return failed(report, format!("failed to construct container ID: {error}")),
    };
    let create_operation_id = match operation_id(&format!("kvm-start-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-start-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("kvm-start-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    if source_marker.exists() {
        return failed(
            report,
            format!(
                "refusing to use a KVM Start qualification bundle with an existing marker: {}",
    ...
    let first_console = prepared.console_directory.join("start-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("start-reopen-replacement.log");
    ...
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM Start reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM Start reopen modified the private bootstrap root {}",
