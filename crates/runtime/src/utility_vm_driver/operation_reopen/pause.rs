...
/// Exact inputs for one real Linux KVM Pause interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmPauseReopenConfig {
    pub shim: PathBuf,
...
/// Rebuild one running or paused container in a distinct KVM owner and replay Pause.
///
/// Create, Start, Pause, exact freezer recovery, stale-generation fencing, and
/// cleanup all use the production immutable-image, exact-generation handoff,
/// and authenticated Guest Agent path. This remains qualification-only and
/// does not register the probe-only KVM candidate.
pub async fn linux_kvm_pause_reopen_replacement(
    config: LinuxKvmPauseReopenConfig,
) -> OciVmOperationReopenReplacementReport {
    let mut report =
        OciVmOperationReopenReplacementReport::initial_pause(HostPlatform::current(), config.stage);
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(
            report,
            "Linux KVM Pause reopen qualification requires Linux",
        );
    }
    ...
    let container_id = match a3s_oci_sdk::ContainerId::new(format!("kvm-pause-reopen-{nonce}")) {
        Ok(id) => id,
        Err(error) => return failed(report, format!("failed to construct container ID: {error}")),
    };
    let create_operation_id = match operation_id(&format!("kvm-pause-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-pause-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let pause_operation_id = match operation_id(&format!("kvm-pause-reopen-{nonce}-pause")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("kvm-pause-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let stale_guest_operation_id =
        match operation_id(&format!("kvm-pause-reopen-{nonce}-stale-guest")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let stale_host_operation_id =
        match operation_id(&format!("kvm-pause-reopen-{nonce}-stale-host")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    ...
    if source_marker.exists() {
        return failed(
            report,
            format!(
                "refusing to use a KVM Pause qualification bundle with an existing marker: {}",
    ...
    report.qualification_operation_id = Some(pause_operation_id.clone());
    ...
    let first_console = prepared.console_directory.join("pause-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("pause-reopen-replacement.log");
    ...
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM Pause reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM Pause reopen modified the private bootstrap root {}",
