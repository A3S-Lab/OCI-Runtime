...
use a3s_oci_sdk::{ErrorCode, ListRequest, OciRuntimeService, ProcessesRequest};
...
use super::support::{
    append_failure, durable_exec_process, exact_process_target, exec_journal_status,
    inventory_matches, path_absent, stale_target, wait_for_exact_marker, ExecJournalStatus,
    FirstOwnerOutcome,
};
...
pub(super) async fn run(
        ...
        || report.replacement_rehydrated_resumed_record
    {
        append_failure(
            &mut failure,
            "replacement KVM driver did not rebuild the exact live init and Exec inventory",
        );
    }
    if report.replacement_created_pid.is_none() || report.replacement_exec_pid.is_none() {
        append_failure(
            &mut failure,
            "replacement KVM Processes recovery did not retain positive init and Exec PIDs",
        );
    }
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
                ...
                || *record.state.pid() != report.replacement_created_pid
            {
                append_failure(
                    &mut failure,
                    format!(
                        "replacement KVM Processes recovery retained invalid {} record with PID {:?}",
                        record.state.status(),
                        record.state.pid()
                    ),
                );
        ...
        Ok(records) => append_failure(
            &mut failure,
            format!(
                "replacement KVM Processes recovery retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut failure,
            format!("failed to inspect recovered KVM Processes record: {error}"),
        ),
    }
    ...
    match wait_for_exact_marker(
        ...
        "replacement KVM Processes init",
    )
    .await
    {
    ...
    match wait_for_exact_marker(
        ...
        "replacement KVM Processes Exec",
    )
    .await
    {
    ...
    .await
    {
        Ok(Ok(inventory)) => {
            ...
            if !report.replacement_process_inventory_verified {
                append_failure(
                    &mut failure,
                    "replacement KVM Processes response did not contain the exact rebuilt inventory",
                );
            }
            if !report.same_generation_reused {
                append_failure(
                    &mut failure,
                    "replacement KVM Processes response changed the durable generation",
                );
            }
            Some(inventory)
        }
        Ok(Err(error)) => {
            append_failure(
                &mut failure,
                format!("replacement KVM Processes failed: {error}"),
            );
            None
        }
        Err(_) => {
            append_failure(&mut failure, "replacement KVM Processes timed out");
            None
        }
    };
    ...
    if report.operation_replayed_without_driver_dispatch {
        append_failure(
            &mut failure,
            "replacement KVM Processes query did not reach the replacement driver",
        );
    }
    if report.replacement_operation_dispatches != 1 {
        append_failure(
            &mut failure,
            format!(
                "replacement KVM driver recorded {} Processes dispatches instead of one",
    ...
    match driver.processes_identity() {
        Ok(identity) => {
            report.processes_request_target_reused =
                identity == first.processes_identity && identity == first.target;
            if !report.processes_request_target_reused {
                append_failure(
                    &mut failure,
                    "replacement KVM Processes query changed its exact resolved target",
                );
            }
        }
        Err(reason) => append_failure(&mut failure, reason),
    ...
    match timeout(
        QUALIFICATION_TIMEOUT,
        driver.guest_processes(AgentProcessesRequest {
            target: stale.clone(),
        }),
    )
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("replacement KVM Guest returned the wrong stale Processes error: {error}"),
        ),
        Ok(Ok(_)) => append_failure(
            &mut failure,
            "replacement KVM Guest accepted stale Processes",
        ),
        Err(_) => append_failure(
            &mut failure,
            "replacement KVM Guest stale Processes check timed out",
        ),
    }
    ...
    match service.processes(ProcessesRequest { target: stale }).await {
        ...
        Err(error) => append_failure(
            &mut failure,
            format!("reopened KVM Host returned the wrong stale Processes error: {error}"),
        ),
        Ok(_) => append_failure(&mut failure, "reopened KVM Host accepted stale Processes"),
    }
    ...
    if !report.marker_absent_after_cleanup || !report.exec_marker_absent_after_cleanup {
        append_failure(
            &mut failure,
            "replacement KVM Processes markers remained after cleanup",
        );
    }
    ...
    if !report.owners_distinct {
        append_failure(
            &mut failure,
            "first and replacement KVM Processes owner identities were not distinct",
        );
    }
