...
pub(super) struct FirstOwnerOutcome {
    ...
    pub(super) resume_identity: ContainerIdentity,
    pub(super) start: StartRequest,
    pub(super) pause: ContainerOperationRequest,
    pub(super) resume: ContainerOperationRequest,
...
) -> Result<FreezerJournalStatus, String> {
    freezer_journal_status(state_root, operation_id, target, "pause").await
}
...
pub(super) async fn resume_journal_status(
    ...
    freezer_journal_status(state_root, operation_id, target, "resume").await
}
...
pub(super) fn record_interruption(
    ...
    if error.code == ErrorCode::Unavailable && error.retryable && expected_operation {
        ...
        Err(format!(
            "first KVM owner returned an unexpected Resume transport error at {}: {error}",
            stage.as_str()
        ))
    }
}
