...
use flow::support::{stdin_exec_process, CLOSE_MARKER_NAME};

/// Exact inputs for one real Linux KVM pipe-backed CloseStdin interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmCloseStdinReopenConfig {
    pub shim: PathBuf,
...
pub(super) struct Qualification {
    ...
    close_operation_id: a3s_oci_sdk::OperationId,
...
/// Rebuild or replay one pipe-backed CloseStdin in a distinct real KVM owner.
///
/// Create, Start, Exec, CloseStdin, process-record rebinding, stale-generation
...
pub async fn linux_kvm_close_stdin_reopen_replacement(
    config: LinuxKvmCloseStdinReopenConfig,
    ...
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(
            report,
            "Linux KVM CloseStdin reopen qualification requires Linux",
        );
    }
    ...
    let container_id =
        match a3s_oci_sdk::ContainerId::new(format!("kvm-close-stdin-reopen-{nonce}")) {
            Ok(id) => id,
            Err(error) => {
                return failed(report, format!("failed to construct container ID: {error}"));
            }
        };
    let create_operation_id = match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-create"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let exec_operation_id = match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-exec")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let close_operation_id = match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-close")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-delete"))
    {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    let stale_guest_operation_id =
        match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-stale-guest")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let stale_host_operation_id =
        match operation_id(&format!("kvm-close-stdin-reopen-{nonce}-stale-host")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    ...
    let source_rootfs = match source_init_marker.parent() {
        Some(rootfs) => rootfs,
        None => return failed(report, "KVM CloseStdin init marker has no rootfs parent"),
    };
    let source_exec_marker = source_rootfs.join(EXEC_MARKER_NAME);
    let source_close_marker = source_rootfs.join(CLOSE_MARKER_NAME);
    for (label, marker) in [
        ("init", &source_init_marker),
        ("Exec", &source_exec_marker),
        ("CloseStdin", &source_close_marker),
    ] {
        if marker.exists() {
            return failed(
                report,
                format!(
                    "refusing to use a KVM CloseStdin qualification bundle with an existing {label} marker: {}",
                    marker.display()
                ),
            );
        }
    ...
    report.qualification_operation_id = Some(close_operation_id.clone());
    ...
    let first_console = prepared
        .console_directory
        .join("close-stdin-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("close-stdin-reopen-replacement.log");
    ...
    report.close_marker_absent_after_cleanup =
        report.close_marker_absent_after_cleanup && !source_close_marker.exists();
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM CloseStdin reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM CloseStdin reopen modified the private bootstrap root {}",
