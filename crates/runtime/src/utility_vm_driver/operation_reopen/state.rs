...
/// Exact inputs for one real Linux KVM State interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmStateReopenConfig {
    pub shim: PathBuf,
...
/// Rebuild one created container in a distinct KVM owner and repeat State.
///
/// The setup Create and replacement recovery use the same production image,
/// handoff, runtime-share, and authenticated Guest Agent path as the public KVM
/// candidate. This remains qualification-only and does not register that
/// candidate for public workload selection.
pub async fn linux_kvm_state_reopen_replacement(
    config: LinuxKvmStateReopenConfig,
) -> OciVmOperationReopenReplacementReport {
    let mut report =
        OciVmOperationReopenReplacementReport::initial_state(HostPlatform::current(), config.stage);
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(
            report,
            "Linux KVM State reopen qualification requires Linux",
        );
    }
    ...
    let container_id = match a3s_oci_sdk::ContainerId::new(format!("kvm-state-reopen-{nonce}")) {
        Ok(id) => id,
        Err(error) => return failed(report, format!("failed to construct container ID: {error}")),
    };
    let create_operation_id = match operation_id(&format!("kvm-state-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let qualification_operation_id =
        match operation_id(&format!("kvm-state-reopen-{nonce}-qualification")) {
            Ok(operation_id) => operation_id,
            Err(reason) => return failed(report, reason),
        };
    let delete_operation_id = match operation_id(&format!("kvm-state-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    if marker.exists() {
        return failed(
            report,
            format!(
                "refusing to use a KVM State qualification bundle with an existing marker: {}",
                marker.display()
            ),
        );
    }
    ...
    let first_console = prepared.console_directory.join("state-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("state-reopen-replacement.log");
    ...
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM State reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM State reopen modified the private bootstrap root {}",
