...
/// Exact inputs for one real Linux KVM Kill interruption and owner reopen.
#[derive(Debug, Clone)]
pub struct LinuxKvmKillReopenConfig {
    pub shim: PathBuf,
...
/// Rebuild one running or stopped container in a distinct KVM owner and replay Kill.
///
/// Create, Start, Kill, marker verification, and stopped-only Delete all use
...
pub async fn linux_kvm_kill_reopen_replacement(
    config: LinuxKvmKillReopenConfig,
) -> OciVmOperationReopenReplacementReport {
    let mut report =
        OciVmOperationReopenReplacementReport::initial_kill(HostPlatform::current(), config.stage);
    if HostPlatform::current() != HostPlatform::Linux {
        return failed(report, "Linux KVM Kill reopen qualification requires Linux");
    }
    ...
    let container_id = match a3s_oci_sdk::ContainerId::new(format!("kvm-kill-reopen-{nonce}")) {
        Ok(id) => id,
        Err(error) => return failed(report, format!("failed to construct container ID: {error}")),
    };
    let create_operation_id = match operation_id(&format!("kvm-kill-reopen-{nonce}-create")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let start_operation_id = match operation_id(&format!("kvm-kill-reopen-{nonce}-start")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let kill_operation_id = match operation_id(&format!("kvm-kill-reopen-{nonce}-kill")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    let delete_operation_id = match operation_id(&format!("kvm-kill-reopen-{nonce}-delete")) {
        Ok(operation_id) => operation_id,
        Err(reason) => return failed(report, reason),
    };
    ...
    if source_marker.exists() {
        return failed(
            report,
            format!(
                "refusing to use a KVM Kill qualification bundle with an existing marker: {}",
    ...
    let first_console = prepared.console_directory.join("kill-reopen-first.log");
    let replacement_console = prepared
        .console_directory
        .join("kill-reopen-replacement.log");
    ...
    match bundle::runtime_inventory(&prepared.runtime_root) {
        ...
        Ok(inventory) => append_reason(
            &mut report,
            format!(
                "KVM Kill reopen left transient runtime state: bundle_handoffs_clean={}, runtime_shares_clean={}, recovery_reports_clean={}, console_files={}",
    ...
    match directory_is_empty(&prepared.bootstrap_root).await {
        ...
        Ok(false) => append_reason(
            &mut report,
            format!(
                "KVM Kill reopen modified the private bootstrap root {}",
