...
use a3s_oci_sdk::{
    ErrorCode, ListRequest, OciRuntimeService, OperationContext, ProcessTarget, WriteStdinRequest,
};
...
use super::support::{
    append_failure, durable_exec_process, path_absent, write_stdin_journal_status,
    FirstOwnerOutcome, WriteStdinJournalStatus,
};
...
pub(super) async fn run(
    ...
    let recovery_write = first.response_delivered.then(|| first.write_stdin.clone());
    ...
    let driver = Arc::new(QualificationKvmOperationDriver::with_write_stdin_recovery(
        prepared,
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                report.write_marker_absent_after_cleanup =
                    path_absent(&first.write_marker).await.unwrap_or(false);
    ...
    if report.replacement_rehydrated_write_stdin != first.response_delivered {
        append_failure(
            &mut failure,
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
                ...
                || *record.state.pid() != report.replacement_created_pid
            {
                append_failure(
                    &mut failure,
                    format!(
                        "replacement KVM WriteStdin recovery retained invalid {} record with PID {:?}",
                        record.state.status(),
                        record.state.pid()
                    ),
                );
        ...
        Ok(records) => append_failure(
            &mut failure,
            format!(
                "replacement KVM WriteStdin recovery retained {} records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut failure,
            format!("failed to inspect recovered KVM WriteStdin record: {error}"),
        ),
    }
    ...
    match wait_for_exact_marker(
        ...
        "replacement KVM WriteStdin init",
    )
    .await
    {
    ...
    if !first.response_delivered {
        match path_absent(&first.write_marker).await {
            Ok(true) => {}
            Ok(false) => append_failure(
                &mut failure,
                "replacement KVM recovery dispatched an uncommitted WriteStdin before API retry",
            ),
            Err(reason) => append_failure(&mut failure, reason),
        }
    }
    ...
    match timeout(
        QUALIFICATION_TIMEOUT,
        service.write_stdin(first.write_stdin.clone()),
    )
    .await
    {
        Ok(Ok(())) => {
            report.operation_completed_after_reopen = true;
            report.generation_after_reopen = first.write_stdin.process.container.generation;
            ...
            if !report.same_generation_reused {
                append_failure(
                    &mut failure,
                    "replacement KVM WriteStdin did not retain the durable generation",
                );
            }
        }
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("replacement KVM WriteStdin failed: {error}"),
        ),
        Err(_) => append_failure(&mut failure, "replacement KVM WriteStdin timed out"),
    }
    ...
    match write_stdin_journal_status(
        state_root,
        &first.write_stdin.context.operation_id,
        &first.write_stdin.process,
    )
    .await
    {
        ...
        Ok(WriteStdinJournalStatus::Prepared) => append_failure(
            &mut failure,
            "replacement KVM WriteStdin journal remained prepared",
        ),
        Err(reason) => append_failure(&mut failure, reason),
    }
    if let Some(process) = replacement_exec.as_ref() {
        match durable_exec_process(state_root, &exact_process).await {
            Ok(durable) => {
                ...
                if !report.exec_response_rebound {
                    append_failure(
                        &mut failure,
                        "completed KVM WriteStdin did not retain the rebound Exec record",
                    );
                }
            }
            Err(reason) => append_failure(&mut failure, reason),
    ...
    .await
    {
        ...
        Ok(Ok(())) => append_failure(
            &mut failure,
            "later KVM WriteStdin replay reached the replacement driver",
        ),
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("later KVM WriteStdin replay failed: {error}"),
        ),
        Err(_) => append_failure(&mut failure, "later KVM WriteStdin replay timed out"),
    }
    ...
    if report.operation_replayed_without_driver_dispatch != first.response_delivered {
        append_failure(
            &mut failure,
            "replacement KVM WriteStdin dispatch did not match the durable journal outcome",
        );
    }
    if report.replacement_operation_dispatches != 1 {
        append_failure(
            &mut failure,
            format!(
                "replacement KVM driver recorded {} WriteStdin dispatches instead of one",
    ...
    match driver.write_stdin_identity() {
        Ok(identity) => {
            ...
            report.same_operation_id_reused = report.write_stdin_request_identity_reused
                && identity.context.operation_id == first.write_stdin.context.operation_id
                && identity.target == exact_process;
            if !report.write_stdin_request_identity_reused || !report.same_operation_id_reused {
                append_failure(
                    &mut failure,
                    "replacement KVM WriteStdin changed its operation, target, or data identity",
                );
            }
        }
        Err(reason) => append_failure(&mut failure, reason),
    ...
    let mut changed_host = first.write_stdin.clone();
    changed_host.data = b"changed-write-stdin\n".to_vec();
    ...
    match service.write_stdin(changed_host).await {
        ...
        Err(error) => append_failure(
            &mut failure,
            format!("reopened KVM Host returned the wrong changed WriteStdin error: {error}"),
        ),
        Ok(()) => append_failure(
            &mut failure,
            "reopened KVM Host accepted changed WriteStdin",
        ),
    }
    ...
    .await
    {
        ...
        Ok(Err(error)) => append_failure(
            &mut failure,
            format!("replacement KVM Guest returned the wrong stale WriteStdin error: {error}"),
        ),
        Ok(Ok(())) => append_failure(
            &mut failure,
            "replacement KVM Guest accepted stale WriteStdin",
        ),
        Err(_) => append_failure(
            &mut failure,
            "replacement KVM Guest stale WriteStdin check timed out",
        ),
    }
        ...
        .await
    {
        ...
        Err(error) => append_failure(
            &mut failure,
            format!("reopened KVM Host returned the wrong stale WriteStdin error: {error}"),
        ),
        Ok(()) => append_failure(&mut failure, "reopened KVM Host accepted stale WriteStdin"),
    }
    ...
    match path_absent(&first.write_marker).await {
        Ok(absent) => report.write_marker_absent_after_cleanup = absent,
        Err(reason) => append_failure(&mut failure, reason),
    }
        ...
        || !report.write_marker_absent_after_cleanup
    {
        append_failure(
            &mut failure,
            "replacement KVM WriteStdin marker remained after cleanup",
        );
    }
    if !report.replacement_guest_runtime_clean {
        append_failure(
            &mut failure,
            "replacement KVM WriteStdin owner left its runtime share",
        );
    }
    if !report.owners_distinct {
        append_failure(
            &mut failure,
            "first and replacement KVM WriteStdin owner identities were not distinct",
        );
    }
