...
use a3s_oci_sdk::{
    ...
    OperationContext, StartRequest, WriteStdinRequest,
};
...
use super::support::{
    ...
    reset_marker, runtime_marker, verify_first_write_marker, write_stdin_journal_status,
    FirstOwnerOutcome, WriteStdinJournalStatus, WRITE_MARKER_NAME,
};
use crate::agent_session::UtilityVmSessionQualification;
use crate::driver::{DriverExecRequest, DriverWriteStdinRequest, RuntimeDriver};
...
pub(super) async fn run(
    ...
    let guest_qualification = if qualification.stage.is_guest() {
        Some(
            AgentTransportQualificationRequest::new(
                qualification.write_operation_id.clone(),
                AgentOperation::WriteStdin,
                qualification.stage,
            )
            .map_err(|error| {
                format!("failed to construct Guest KVM WriteStdin qualification: {error}")
            })?,
        )
    } else {
        None
    ...
    .await
    {
        ...
        Ok(Ok(record)) => {
            ...
            return setup_failure(
                ...
                report,
                format!(
                    "KVM WriteStdin setup Create returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            ...
            return setup_failure(
                &driver,
                report,
                format!("KVM WriteStdin setup Create failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return setup_failure(
                &driver,
                report,
                "KVM WriteStdin setup Create timed out".to_string(),
            )
            .await;
        }
    };
    ...
    let write_marker = match process_marker(&init_marker, WRITE_MARKER_NAME, "WriteStdin") {
        Ok(marker) => marker,
        Err(reason) => {
            drop(service);
            return active_failure(&driver, &target, report, reason).await;
        ...
        match path_absent(marker).await {
            ...
            Ok(false) => {
                ...
                return active_failure(
                    ...
                    report,
                    format!(
                        "KVM WriteStdin {label} marker existed before setup Start: {}",
                        marker.display()
                    ),
                )
                .await;
    ...
    let started = match timeout(QUALIFICATION_TIMEOUT, service.start(start.clone())).await {
        ...
        Ok(Ok(record)) => {
            ...
            return active_failure(
                ...
                report,
                format!(
                    "KVM WriteStdin setup Start returned invalid {} record with PID {:?}",
                    record.state.status(),
                    record.state.pid()
                ),
            )
        ...
        Ok(Err(error)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM WriteStdin setup Start failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM WriteStdin setup Start timed out".to_string(),
            )
            .await;
        }
    };
    if let Err(reason) = super::super::super::exec::wait_for_exact_marker(
        ...
        "first-owner KVM WriteStdin init",
    )
    .await
    {
        drop(service);
    ...
    let first_exec = match timeout(QUALIFICATION_TIMEOUT, service.exec(exec.clone())).await {
        ...
        Ok(Ok(process)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM WriteStdin setup Exec returned invalid process {process:?}"),
            )
            .await;
        }
        Ok(Err(error)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                format!("KVM WriteStdin setup Exec failed: {error}"),
            )
            .await;
        }
        Err(_) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM WriteStdin setup Exec timed out".to_string(),
            )
            .await;
        }
    };
    ...
    match (
        ...
        (Ok(ProcessOperationJournalStatus::Succeeded(_)), Ok(_)) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM WriteStdin setup Exec journal changed its durable process".to_string(),
            )
            .await;
        }
        (Ok(ProcessOperationJournalStatus::Prepared), _) => {
            ...
            return active_failure(
                ...
                &target,
                report,
                "KVM WriteStdin setup Exec journal remained prepared".to_string(),
            )
            .await;
        }
    ...
    let write_stdin = WriteStdinRequest {
        context: OperationContext::new(qualification.write_operation_id.clone()),
        process: exact_process.clone(),
        data: qualification.write_data.clone(),
    };
    ...
    match timeout(
        QUALIFICATION_TIMEOUT,
        service.write_stdin(write_stdin.clone()),
    )
    .await
    {
        Ok(Err(error)) => {
        ...
        Ok(Ok(())) => append_failure(
            &mut failure,
            "first KVM WriteStdin unexpectedly completed before owner replacement",
        ),
        Err(_) => append_failure(&mut failure, "first KVM WriteStdin timed out"),
    }
    ...
    match service.list(ListRequest::default()).await {
        Ok(records) if records.len() == 1 => {
            ...
            if !report.durable_running_retained {
                append_failure(
                    &mut failure,
                    "interrupted KVM WriteStdin did not retain the exact durable running record",
                );
            }
        }
        Ok(records) => append_failure(
            &mut failure,
            format!(
                "interrupted KVM WriteStdin retained {} durable records instead of one",
                records.len()
            ),
        ),
        Err(error) => append_failure(
            &mut failure,
            format!("failed to inspect durable state after interrupted KVM WriteStdin: {error}"),
        ),
    }
    match write_stdin_journal_status(
        state_root,
        &write_stdin.context.operation_id,
        &write_stdin.process,
    )
    .await
    {
        Ok(WriteStdinJournalStatus::Prepared) => {
            ...
            if response_delivered {
                append_failure(
                    &mut failure,
                    "delivered first KVM WriteStdin response left its journal prepared",
                );
            }
        }
        Ok(WriteStdinJournalStatus::SucceededEmpty) => {
            ...
            if !response_delivered {
                append_failure(
                    &mut failure,
                    "KVM WriteStdin journal committed before its response boundary",
                );
            }
        }
        Err(reason) => append_failure(&mut failure, reason),
    ...
    let expected_driver_write = DriverWriteStdinRequest {
        context: write_stdin.context.clone(),
        target: exact_process,
        data: write_stdin.data.clone(),
    };
        ...
        match read_guest_qualification_evidence(first_console, request).await {
            Ok(evidence) => {
                ...
                if !report.guest_evidence_verified {
                    append_failure(
                        &mut failure,
                        "Guest KVM WriteStdin evidence did not match the exact qualification",
                    );
                }
            }
            Err(reason) => append_failure(&mut failure, reason),
    ...
    for (marker, field) in [
        ...
        (
            &write_marker,
            &mut report.write_marker_reset_before_replacement,
        ),
    ] {
    ...
    if !report.first_guest_runtime_clean {
        append_failure(
            &mut failure,
            "first KVM WriteStdin owner left Guest Agent runtime state",
        );
    }
    for (label, actual) in [
    ...
    if report.fault_crossings != 1 {
        append_failure(
            &mut failure,
            format!(
                "selected KVM WriteStdin point crossed {} times instead of once",
    ...
    let write_stdin_identity = match write_stdin_identity {
        Ok(identity) => identity,
        Err(reason) => {
            append_failure(&mut failure, reason);
