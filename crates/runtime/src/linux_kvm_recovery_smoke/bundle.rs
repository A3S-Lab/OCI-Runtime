...
pub(crate) async fn stage(
    ...
    std::fs::create_dir(&destination).map_err(|error| {
        format!(
            "failed to create bundle handoff {}: {error}",
            destination.display()
        )
    })?;
    std::fs::set_permissions(&destination, std::fs::Permissions::from_mode(0o700)).map_err(
        |error| {
            format!(
                "failed to protect bundle handoff {}: {error}",
                destination.display()
            )
        },
    )?;
    ...
    let status = Command::new("/bin/cp")
        ...
        .status()
        .await
        .map_err(|error| format!("failed to copy bundle handoff: {error}"))?;
    if !status.success() {
        return Err(format!(
            "cp failed to stage bundle handoff with status {status}"
        ));
    }
...
pub(crate) fn runtime_inventory(runtime_root: &Path) -> Result<RuntimeInventory, String> {
    Ok(RuntimeInventory {
        ...
        recovery_reports_clean: directory_is_empty(&runtime_root.join("recovery"))?,
        console_files: file_count(&runtime_root.join("console"))?,
    })
}
...
fn file_count(path: &Path) -> Result<u32, String> {
    ...
    for entry in entries {
        let entry = entry.map_err(|error| format!("failed to inspect console file: {error}"))?;
        if entry
            .metadata()
            ...
            .is_file()
        {
            count = count
                .checked_add(1)
                .ok_or_else(|| "console file count overflowed".to_string())?;
        }
    }
    Ok(count)
}
