...
Native Linux soak schema v2 now retains every Pause and Resume operation ID,
replays each committed response after a separate Host Service reopen, and
records atomic workload counters proving no progress while paused and renewed
progress after Resume and its second reopen. Tagged package qualification v4
binds this OAR-02 evidence to the exact staged runtime and Agent artifacts.
...
The September 6–7, 2026 current-host operation qualification extended that
evidence to every workload operation. Seven bounded runs on Windows 10 Pro
23H2 (AMD64) passed 180/180 operation-stage cases (20 operations x 9 retained
fault stages), including the clock-adjusted Stats replacement case; all
reports retained expected owner/fault crossings, immutable asset hashes, and
complete process/share cleanup. The independent
`a3s.oci.windows-whpx-handle-reclamation-run.v1` gate also passed eight
same-process VM cycles with `115 -> 122 -> 122` cold/baseline/final handles,
...
All retained Linux KVM entry, compatibility, lifecycle, recovery,
operation-reopen, and soak artifacts carry the shared provenance contract
described below. This closes artifact identity ambiguity; it does not
substitute unavailable-runner output for successful real-KVM evidence.
...
for host-kernel execution. Box main commit
`d6861de302e6e165a2fdc473b2d399bb0692048e` retained that installed-product
boundary in
[CI run 33497670646](https://github.com/A3S-Lab/Box/actions/runs/33497670646)
on x86_64 and aarch64 against Runtime commit
...
replacement paths. The September 3 follow-up at clean Runtime revision
`fa4c593` added real File and Filesystem owner replacement, passing 9/9 stages
for each operation (18/18 additional paths) with immutable asset provenance
and zero residue. On the existing Windows 10 x86_64 host, the pinned WHPX
...
architectures, the remaining WHPX/KVM operation-stage and shutdown boundaries, and signed
release artifacts are still required before promotion. A subsequent run from
merged Runtime commit `bf43388dc1a5630f3fbbd699203877cf84f1ee2d` repeated the
WHPX 56/56 soak, direct-driver, and service-recovery gates against a
source-matched immutable image; all 51 VM handle inventories were restored and
the host process inventory returned to zero. It remains observation-only on the
existing host and therefore does not close the freshly provisioned release-host
or operation-stage gates.
...
On September 11, 2026, tip `ff97257` retained existing-host WSL2 x86_64
`/dev/kvm` observation for the Linux KVM release matrix with
`host_class=existing` and `A3S_OCI_LINUX_KVM_SKIP_SOAK=1`: agent-entry,
compatibility-drift, lifecycle, recovery, and create-reopen all `available`
(`promotes_readiness=false`; report SHA-256
`1bd77d6c9091168220ce4d35d0d5f19d1a381498f5b048c5588761c8efb09ccd`). The same
...
| Tip-matched promote on current `main` | **Hardware-blocked** — tip `139850c` (Merge #341, archive `v0.3.5`) is fmt-and-clippy-clean on pinned rustc `1.98.1` ([CI 34680331496](https://github.com/A3S-Lab/OCI-Runtime/actions/runs/34680331496)). No reachable self-hosted WHPX/KVM runners from this environment. Does not promote readiness |
...
State uses the same qualification scope after an exact setup Create. It
rebuilds that Created container in a distinct replacement Guest and reissues
State against the original durable generation:
...
Start retains the same setup Create and exact Start identity. A replacement
Guest either dispatches the prepared Start once or reconstructs an already
committed Running state before the Host replays the durable response:
...
Kill retains the exact setup Create, Start, and signal-9 Kill identities. A
replacement Guest reconstructs the Running workload and either dispatches the
prepared Kill once or rebuilds an already committed Stopped tombstone before
the Host replays the durable response:
...
Delete retains those three setup identities plus the exact stopped-only
Delete identity. Its first eight paths rebuild the Stopped tombstone and
dispatch Delete once. The final committed path starts a distinct empty KVM
owner, rebuilds no workload, and lets the Host replay the completed journal
without another driver dispatch:
...
Wait uses the same exact stopped setup and resolves the current target to its
durable generation. The first eight paths rebuild the Stopped Guest tombstone,
dispatch the original 15-second Wait once, and cache its signal-9 result. The
committed final path already has that cache, so both replacement and later Wait
calls replay without another driver or Guest dispatch:
...
Exec retains the exact setup Create and Start identities plus one nonce-bound,
long-running terminal process request. The first eight paths recover only the
Running init process and dispatch the unchanged Exec once after the API retry.
The committed final path recreates both init and Exec during recovery, rebinds
their positive PIDs into the durable responses, and lets the Host replay Exec
without another API-driven dispatch:
...
SignalProcess retains that committed terminal Exec and sends exact signal 10.
The first eight paths rebuild init and Exec but leave the Prepared signal for
the API retry to dispatch once. The committed final path waits for the rebuilt
Exec readiness marker, reapplies the signal once during recovery, and lets the
Host replay the completed journal without another driver dispatch:
...
WaitProcess terminates the same committed non-init Exec with signal 10 and
waits up to 15 seconds for its exact exit. The first eight paths rebuild and
terminate the Exec, then dispatch the resolved WaitProcess target once on API
retry and cache `signal=10, oom_killed=false`. The committed final path already
has that durable cache, so replacement and later WaitProcess calls perform no
driver dispatch:
...
Pause retains the exact setup Create and Start identities and verifies the
nonce-bound init marker before freezing the generation. The first eight paths
rebuild an unpaused init and dispatch the unchanged Pause once on API retry.
The committed final path reapplies Pause during recovery, rebinds the paused
record to the replacement PID, and lets the Host replay without another
API-driven dispatch:
...
Resume retains the exact setup Create, Start, and Pause identities. Every
replacement Guest reconstructs the freezer history after the nonce-bound init
marker. The first eight paths dispatch the unchanged Resume once on API retry;
the committed final path reapplies Resume during recovery and lets the Host
replay without another API-driven dispatch:
...
identities. Every replacement Guest recreates init and Exec with fresh positive
PIDs and both nonce-bound markers, then receives the read-only Processes query
once because no durable query-response journal exists. The returned inventory
must contain exactly those two targets at the retained generation, including
after the first owner wrote a complete response:
...
Update retains the exact setup Create and Start identities plus a complete
Linux resource profile. The first eight replacement Guests receive the
unchanged request once after rebuilding the running init. When the first owner
already committed the response, recovery reapplies the resource profile to the
fresh cgroup and the Host replays the response without another API-driven
dispatch. Direct Stats verifies the 512 MiB limit and live counters:
...
ReadOutput retains the committed Create, Start, and non-terminal capture Exec
setup. Every replacement Guest receives one fresh request with the same exact
process target, cursor, byte limit, and timeout, including after the first
owner delivered the complete nonce-bound stdout chunk. Recovery rebinds both
setup PIDs, fences stale Host and Guest generations, and removes both markers
and all transient owner state:
...
WriteStdin retains the committed Create, Start, and non-terminal pipe-backed
Exec setup. The first eight replacement owners dispatch the unchanged bytes
once from the Prepared Host journal. At `guest-after-response-write`, recovery
rehydrates the committed write into the rebuilt Exec and the API retry returns
without another driver dispatch. Every path verifies the exact effect marker,
request identity, stale Host and Guest fences, and complete cleanup:
...
CloseStdin retains the same setup and a pipe-backed Exec that writes its exact
effect marker only after observing EOF. The first eight replacement owners
dispatch the unchanged close once from the Prepared Host journal. At
`guest-after-response-write`, recovery closes the rebuilt Exec input before
Host service open completes, so the API retry returns without another driver
dispatch. Every path verifies the exact process target, EOF marker, stale Host
and Guest fences, and complete cleanup:
...
Resize retains the exact terminal dimensions and a nonce-bound PTY Exec. The
first eight replacement owners dispatch the prepared resize once; the final
path reapplies committed dimensions during recovery and replays the Host
response without a second driver dispatch:
...
File qualifies an upload through the same nine Host/Guest transport boundaries
using a private writable `/tmp` mount. Recovery validates the exact durable
request and generation, replays an acknowledged upload idempotently, downloads
the bytes from the replacement Guest, rejects changed and stale identities, and
removes the file before force-delete:
...
Filesystem qualifies mkdir with the corresponding directory metadata and Stat
effect. It uses the same durable journal, replacement-owner replay, changed and
stale-generation fences, explicit Remove, and zero-residue checks:
...
inventories. The `a3s.oci.linux-kvm-state-reopen-matrix.v1` gate applies the
same nine points to State. Its first eight points retain the exact Created
record without returning a response; `guest-after-response-write` delivers an
exact response before a disconnect probe proves owner loss. Replacement
recovery must rebuild Created state, preserve the setup Create identity and
generation, return the recovered durable record, and restore the same cleanup
inventories. The `a3s.oci.linux-kvm-start-reopen-matrix.v1` gate then applies
the same points to Start. Its first eight paths retain Created state and
dispatch the exact Start once through the replacement driver. The final path
retains Running state; recovery recreates and starts the workload, rebinds the
replacement PID, regenerates the exact marker, and lets the Host replay the
completed journal without another API-driven Start dispatch. The
`a3s.oci.linux-kvm-kill-reopen-matrix.v1` gate applies the same nine points to
Kill. Its first eight paths retain Running state; replacement recovery
recreates and starts the workload, repairs the setup responses, and dispatches
the unchanged SIGKILL once. The final path retains Stopped state; recovery
recreates, starts, and kills the replacement workload to reconstruct the Guest
tombstone, then the Host replays the completed Kill journal without another
API-driven dispatch. Every path verifies the replacement marker and uses
stopped-only Delete. The `a3s.oci.linux-kvm-delete-reopen-matrix.v1` gate then
applies all nine points to Delete. Its first eight paths retain Stopped state
and a Prepared journal; replacement recovery recreates, starts, and kills the
workload with the original setup identities before dispatching the unchanged
Delete once. The committed final path retains an empty inventory and a
SucceededEmpty journal, so a distinct empty replacement owner performs no
workload recovery or driver Delete before Host replay. The
`a3s.oci.linux-kvm-wait-reopen-matrix.v1` gate rebuilds the stopped tombstone
at all nine points. Its first eight paths dispatch the exact resolved Wait
target once and durably cache `signal=9, oom_killed=false`; the committed final
path retains that cache and replays both replacement and later Wait calls
without a driver dispatch. Every path rejects stale generations at both Host
and Guest boundaries, uses stopped-only Delete, and restores all inventories.
The `a3s.oci.linux-kvm-exec-reopen-matrix.v1` gate applies all nine points to a
terminal Exec. Its first eight paths retain a Prepared journal, recover only
the Running init process, and dispatch the byte-identical process, terminal,
I/O, generation, and operation identity once. The committed final path retains
the Succeeded process journal, recreates both init and Exec with fresh positive
PIDs, rebinds the setup and Exec responses, and replays the Host request without
another driver dispatch. Every path verifies distinct nonce-bound init and Exec
markers, rejects changed requests and stale generations, force-deletes the
workload, and restores all inventories. The
`a3s.oci.linux-kvm-signal-process-reopen-matrix.v1` gate retains that exact
terminal Exec and applies signal 10 at all nine points. Its first eight paths
retain a Prepared signal journal, rebuild init and Exec, and dispatch the
unchanged target and signal once on API retry. The committed final path retains
a SucceededEmpty journal, rebuilds init and Exec, waits for the nonce-bound Exec
readiness marker, and reapplies the signal exactly once during recovery; Host
replay then performs no additional driver dispatch. Every path verifies the
separate signal marker, rejects signal drift and stale Host/Guest generations,
force-deletes the workload, and restores all inventories. Equivalent coverage
for WaitProcess uses
`a3s.oci.linux-kvm-wait-process-reopen-matrix.v1`. Its first eight paths have
no Host exit cache, so recovery rebuilds and terminates the exact Exec before
one resolved WaitProcess dispatch persists `signal=10, oom_killed=false`. At
`guest-after-response-write`, the cache is already durable; recovery still
recreates and terminates the Exec but omits it from the live inventory, and
both replacement and later waits replay with zero driver dispatch. Every path
reuses all setup identities, rejects stale Host and Guest generations,
force-deletes the workload, and restores every inventory. Equivalent coverage
for Pause uses `a3s.oci.linux-kvm-pause-reopen-matrix.v1`. Its first eight
paths retain Running state and a Prepared Pause journal, rebuild the unpaused
init, and dispatch the unchanged Pause once. The committed final path retains
paused state and a Succeeded journal; recovery starts the replacement init,
waits for its marker, reapplies Pause, and reports recreated-paused-running
evidence before Host replay. Every path fences changed requests and stale Host
and Guest generations, force-deletes the paused generation, and restores every
inventory. Resume uses `a3s.oci.linux-kvm-resume-reopen-matrix.v1`. Every
replacement owner reconstructs Create, Start, and Pause with rebound PIDs. Its
first eight paths retain paused state and a Prepared Resume journal before one
unchanged dispatch. The committed final path retains unpaused state and a
Succeeded journal; recovery reapplies Resume and the Host replay performs no
additional API-driven dispatch. Every path fences changed requests and stale
Host and Guest generations, force-deletes the resumed generation, and restores
every inventory. Processes uses
`a3s.oci.linux-kvm-processes-reopen-matrix.v1`. Every replacement owner
reconstructs Create, Start, and the committed terminal Exec, rebinds both
positive process PIDs, and verifies the two nonce-bound markers. Because the
query is read-only, all nine paths dispatch Processes exactly once after
reopen, including the delivered final response path. Each inventory contains
only init and the original Exec target at the retained generation with the
replacement PIDs. Every path rejects stale Host and Guest generations,
force-deletes the workload, and restores every inventory. Update uses
`a3s.oci.linux-kvm-update-reopen-matrix.v1`. Its first eight paths preserve a
Prepared journal and dispatch the unchanged complete Linux resource request
once after recovery. The committed final path preserves a Succeeded journal;
recovery reapplies the request to the fresh cgroup and Host replay performs no
...
inventory. ReadOutput uses
`a3s.oci.linux-kvm-read-output-reopen-matrix.v1`. Every replacement owner
reconstructs Create, Start, and the live non-terminal Exec with rebound PIDs,
then dispatches one fresh query with the exact process target, cursor, byte
limit, and timeout. At `guest-after-response-write`, both the delivered first
chunk and the replacement chunk equal the nonce-bound stdout. Every path
rejects stale Host and Guest generations, force-deletes the workload, and
restores every inventory. WriteStdin uses
`a3s.oci.linux-kvm-write-stdin-reopen-matrix.v1`. Its first eight paths retain
a Prepared Host journal and dispatch the exact bytes once after recovery. At
`guest-after-response-write`, recovery writes the committed bytes into the
rebuilt pipe-backed Exec and the API retry performs no additional dispatch.
Every path rejects changed bytes and stale Host and Guest generations, verifies
the nonce-bound effect marker, force-deletes the workload, and restores every
inventory. CloseStdin uses
`a3s.oci.linux-kvm-close-stdin-reopen-matrix.v1`. Its first eight paths retain
a Prepared Host journal and dispatch the exact EOF once after recovery. At
`guest-after-response-write`, recovery closes the rebuilt pipe-backed Exec
and the API retry performs no additional dispatch. Every path rejects a
changed process target and stale Host and Guest generations, verifies the
nonce-bound EOF marker, force-deletes the workload, and restores every
inventory. Resize, File, and Filesystem use the same nine-stage gate; current
x86_64 evidence now covers all 20 workload operations. Host shutdown remains
...
a paused running record. Terminal restore failures quarantine only their
allocated generation so the ID can be reused monotonically. The registry
accepts `Checkpoint` only from an explicitly advertising current-platform
driver and accepts `Restore` only together with `Checkpoint`. The default
Native Linux inventory and ordinary experimental constructor advertise
neither operation. The separate rootful `open_experimental_with_criu`
constructor binds one exact CRIU executable and advertises Checkpoint and
Restore.
Its `native-linux-criu` v1 backend advertises both operations, atomically
publishes one streaming digest-bound artifact, recreates a newer exact paused
generation through CRIU, replays checkpoint/restore response loss through the
Host boundary, leaves the source paused, and has a bounded real-kernel positive
gate plus private-PID- and configured-network-namespace negative gates. Its v3
gate also replaces the runtime owner after the Restore driver call and after
the completed Host-operation directory sync; a fresh service and driver reopen
the same roots, recreate a live paused generation from retained immutable
images, replay the exact response, preserve the artifact, and clean every
...
| Official OCI Runtime Tools bundle gate | Runtime Tools 0.9.0 at `8a4db579f5c88af5a0d036fad34bddc9c1f703f3` · OCI 1.3.0 Native Linux and utility-VM bundles · MUST level · escaping-rootfs negative |
...
- Native Linux CRIU broader checkpoint source profiles, cross-driver and
  retained tagged multi-architecture real-host qualification, and production
