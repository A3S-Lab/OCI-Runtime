...
| containerd runtime-v2 | SDK-only `containerd-shim-a3s-oci-v2` with durable namespace/task incarnation mapping, lifecycle and exec recovery, schema-v6 init/exec input and terminal-resize journals, Open/Closing/Closed stdin state, output cursors, per-process resize sequences, and a per-task control sequence, process/task-scoped serialization, cross-process-stable request fingerprints, bounded FIFO/PTY I/O, live shim replacement with exact stdin continuation, committed pending-write, close, and resize replay without duplicate effects, same-size resize suppression, correct `A→B→A` terminal restoration, and no output replay, repeated pause/resume and update, stats, PID inventory, in-flight Create and committed Start/Kill/Delete/Exec/SignalProcess/Pause/Resume/Update/WriteStdin/CloseStdin/ResizePty recovery plus four-state forced shim-crash cleanup, and a four-task parallel restart gate; compatibility, packaging, and cross-driver release gates remain open |
