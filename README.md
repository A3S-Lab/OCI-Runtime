...
| containerd runtime-v2 | SDK-only `containerd-shim-a3s-oci-v2` with durable namespace/task incarnation mapping, lifecycle and exec recovery, schema-v2 init/exec output cursors, bounded FIFO/PTY I/O, live shim replacement without output replay, close-I/O, resize, pause/resume, update, stats, PID inventory, in-flight Create and committed Start/Kill/Delete recovery plus four-state forced shim-crash cleanup, and a four-task parallel restart gate; compatibility, packaging, and cross-driver release gates remain open |
