...
restore journal, staging, executor, and session path. Package qualification v4
...
`qualification/native-linux-package.json` schema v4 plus eleven digest-bound
...
rejection gates with the staged binaries. The workflow builds upstream CRIU
v4.2.1 at pinned commit
`9539417f3e3cfa4eb84c319cd71f4d52f1f08645`; CRIU remains host-provided and
outside the archive, while its exact version, Git ID, digest, and size are
bound by the package report. Package availability never overrides the
