...
| macOS arm64/HVF | Real HVF object lifecycle, pinned libkrun context and guest entry, protocol-v9 agent, fixed and multi-container lifecycle, descriptor-confined filesystem sessions, mount/namespace profiles, no-delete cleanup, four host-side Create transport interruption/cleanup stages, and 25 fresh-VM waves | `probe-only`; immutable system image, guest-side and host-service-reopen fault matrices, and release hardware qualification remain |
...
| Real HVF Create host-transport interruption and cleanup stages | 4 |
...
- the remaining real utility-VM guest-side and host-service-reopen transport
