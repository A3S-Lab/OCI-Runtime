...
| Retained evidence | Schema and normative locks, exhaustive durable and authenticated agent fault matrices, portable nine-stage Create/State/Start/Kill/Delete/Wait/Exec/SignalProcess/WaitProcess/Pause/Resume/Processes/Update/Stats/ReadOutput/WriteStdin/CloseStdin/Resize/File/Filesystem host reopen, real-HVF nine-stage Host/Guest Create plus two-stage Host shutdown interruption and cleanup, one real-HVF durable Create reopen through VM/session-owner replacement, native Linux real-container, soak, and owner-death safe-termination gates, fresh-VM HVF soak, and WHPX nominal plus owner-death/service-restart qualification |
...
| macOS arm64/HVF | Real HVF object lifecycle, pinned libkrun context and guest entry, protocol-v9 agent, fixed and multi-container lifecycle, descriptor-confined filesystem sessions, mount/namespace profiles, no-delete cleanup, all nine Host/Guest Create transport stages, both Host shutdown stages, one durable Create resumed through Host-service reopen and a fresh VM/session owner, and 25 fresh-VM waves | `probe-only`; immutable system image, the remaining reopen/replacement fault matrix, and release hardware qualification remain |
...
| Real HVF durable Host reopen plus VM/session-owner replacement paths | 1 |
