...
| containerd runtime-v2 | SDK-only `containerd-shim-a3s-oci-v2` with durable namespace/task incarnation mapping, lifecycle and exec recovery, schema-v4 init/exec input journals and output cursors plus a per-task control sequence, task-scoped control serialization, cross-process-stable request fingerprints, bounded FIFO/PTY I/O, live shim replacement with exact stdin continuation, committed pending-write replay without duplicate input, and no output replay, close-I/O, resize, repeated pause/resume and update, stats, PID inventory, in-flight Create and committed Start/Kill/Delete/Exec/SignalProcess/Pause/Resume/Update/WriteStdin recovery plus four-state forced shim-crash cleanup, and a four-task parallel restart gate; compatibility, packaging, and cross-driver release gates remain open |
