...
| macOS arm64/HVF | Real HVF object lifecycle, pinned libkrun context and guest entry, protocol-v9 agent, fixed and multi-container lifecycle, descriptor-confined filesystem sessions, mount/namespace profiles, no-delete cleanup, all nine Host/Guest Create transport stages, both Host shutdown stages, every Create, State, Start, Kill, Delete, Wait, Exec, SignalProcess, WaitProcess, Pause, Resume, Processes, Update, Stats, ReadOutput, WriteStdin, CloseStdin, Resize, and File transition resumed or reissued through Host-service reopen and a fresh VM/session owner, and 25 fresh-VM waves | `probe-only`; immutable system image, the remaining Filesystem reopen/replacement matrix, and release hardware qualification remain |
...
| Real HVF durable operation replacement coverage | 171 / 180 paths (19 / 20 operations) |
...
- the remaining Filesystem real utility-VM host-service-reopen and replacement
