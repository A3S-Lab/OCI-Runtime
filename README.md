...
| macOS arm64/HVF | Real HVF object lifecycle, pinned libkrun context and guest entry, protocol-v8 agent, fixed and multi-container lifecycle, mount/namespace profiles, no-delete cleanup, and 25 fresh-VM waves | `probe-only`; immutable system image, exhaustive recovery, and release hardware qualification remain |
| Windows x86_64/WHPX | Real partition/context/guest gates, protocol-v8 lifecycle, direct driver qualification, protected per-generation shares, exact exit replay, owner death at both recovery fault boundaries, host-service reopen, stopped-only delete, and complete transient cleanup | `probe-only`; pinned immutable system root and in-process native-handle reclamation remain before `experimental` |
...
```text
A3S Box (current Sandbox consumer; unified cutover in progress)
a3s-oci CLI
