...
| macOS arm64/HVF | Real HVF object lifecycle, pinned libkrun context and guest entry, protocol-v9 agent, fixed and multi-container lifecycle, descriptor-confined filesystem sessions, mount/namespace profiles, no-delete cleanup, all nine Host/Guest Create transport interruption/cleanup stages, and 25 fresh-VM waves | `probe-only`; immutable system image, Host shutdown and host-service-reopen fault matrices, and release hardware qualification remain |
...
| Real HVF Create Host/Guest transport interruption and cleanup stages | 9 |
...
- the remaining real utility-VM Host shutdown, host-service-reopen, and
  replacement transport matrix plus hook recovery/security certification;
