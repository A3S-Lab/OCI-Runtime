...
restore journal, staging, executor, and session path. Package qualification v5
...
digests are bound by the package report. This bundle gate does not qualify the
still-open OCI Runtime Command Line Interface lifecycle adapter or its
every-platform exact-package runs. Package availability never overrides the
