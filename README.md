...
progress after Resume and its second reopen. Tagged package qualification v3
...
restore journal, staging, executor, and session path. Broader namespace and
descriptor profiles, published-package execution, cross-driver and
...
- Native Linux CRIU broader checkpoint source profiles, published-package,
  cross-driver and multi-architecture real-host qualification, and production
...
reported by the exact binary's `features` result. The host-provided CRIU
checkpoint gate is separate and is not yet included in package qualification
v3.
