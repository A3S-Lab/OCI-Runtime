...
| macOS arm64/HVF | Real HVF object lifecycle, pinned libkrun context and guest entry, protocol-v9 agent, fixed and multi-container lifecycle, descriptor-confined filesystem sessions, mount/namespace profiles, no-delete cleanup, all nine Host/Guest Create transport stages, both Host shutdown stages, every Create transition resumed or rehydrated through Host-service reopen and a fresh VM/session owner, and 25 fresh-VM waves | `probe-only`; immutable system image, the remaining-operation reopen/replacement matrix, and release hardware qualification remain |
...
- the remaining-operation real utility-VM host-service-reopen and replacement
