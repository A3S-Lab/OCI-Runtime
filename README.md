...
constructor binds one exact CRIU executable and advertises Checkpoint only.
Its `native-linux-criu` v1 backend atomically publishes one streaming,
digest-bound artifact, replays an interrupted driver/Host commit through a
private journal, leaves the source paused, and has a bounded real-kernel
positive gate plus a private-PID-namespace negative gate. Restore, broader
...
- Native Linux CRIU Restore, broader checkpoint source profiles,
  published-package and multi-architecture real-host qualification, and
  production checkpoint/restore readiness;
