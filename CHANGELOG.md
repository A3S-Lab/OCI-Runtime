...
- Opt-in Linux KVM Live Host reopen evidence gate (observation-only): schema
  ...
  `linux-kvm-filesystem-reopen` (`live_vm_processes_reaped` must be false). Does
  **not** claim Box filesystem Live / `retained_stream_handle_proven` or flip
  W2/B2 / cutover flags. Prior Host survival-only greening used smoke v1 on
  `c3b5fc54…`; v2 process/I/O greened on existing-host WSL2 `/dev/kvm` at
  `a7e60d06…` (matrix SHA-256
  `888623a38d8399ff65047a5f1e294cbdff2d77b54302172afeab3c081a2ce88c`). v3 adds
  filesystem continuity evidence; filesystem greening remains pending until
  `/dev/kvm` re-run. W2/B2 and cutover flags remain open.
