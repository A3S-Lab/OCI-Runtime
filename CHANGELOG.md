...
- Windows/WHPX durable session-owner host-control bridge keeps the guest agent
  named pipe connected across Host-control EOF (Host taskkill → Live reopen).
  Previously both sides were dropped, which tore down the WHPX shim and defeated
  reattach. Durable AgentVmSession also sets `A3S_OCI_GUEST_HOST_RECONNECT=1`
  and the Windows agent-vm smoke path forwards it into the Linux guest (parity
  with KVM). Does **not** tip-prove Box binder gate 9 or claim Enterprise GA.
...
- Windows `a3s-oci-krun-shim session-owner` / `session-owner-probe` /
  `session-owner-bridge-echo` with Job Object `KILL_ON_JOB_CLOSE` and optional
...
- Release packaging installs musl targets on the pinned 1.98.1 toolchain, matching
  CI, instead of installing them on stable while `rust-toolchain.toml` selects
  1.98.1.
...
- Public agent recovery docs no longer link to private executor types, so
  `cargo doc --workspace --no-deps` passes with warnings denied.
...
- Retained existing-host WSL2 x86_64 `/dev/kvm` tip `05a3b2b` complete
  ...
  (`dc4a2f49…`) plus 19×9/9 operation-stage matrices for the tip-source
  **180/180** Host/Guest paths. Aggregate SHA-256
  `e3c0f48ac002026ddd9ee793a30390e120422a7dfc3f49b5a84938fd0c87396d`
  (`host_class=existing`, `promotes_readiness=false`). Observation-only;
  does not close fresh-host promotion, AArch64, or B2 cutover.
- Retained existing-host WSL2 x86_64 `/dev/kvm` tip `a47b799` R1 operation-
  ...
  (`592cc7f6…`); Filesystem 9/9 (`36862d1d…`). Superseded for the tip
  180/180 claim by the `05a3b2b` aggregate above.
...
- Opt-in Linux KVM Live Host reopen evidence gate (observation-only): schema
  `a3s.oci.linux-kvm-live-recovery-smoke.v3`, CLI
  ...
  Capture stdout) → FileOp::Upload → Host SIGKILL → Guest / session-owner
  survival → replacement Host Running reattach with continuous init identity,
  the same exec process ID, post-reattach write_stdin/read_output
  (`retained_exec_io_proven`), exact FileOp::Download match on the same
  Running generation (`retained_filesystem_proven`), and no invented exit.
  Distinct from stopped-only `linux-kvm-recovery` and from stopped-only
  `linux-kvm-filesystem-reopen` (`live_vm_processes_reaped` must be false).
  Prior Host survival-only greening used smoke v1 on `c3b5fc54…`; v2
  process/I/O greened on existing-host WSL2 `/dev/kvm` at `a7e60d06…`
  (matrix SHA-256
  `888623a38d8399ff65047a5f1e294cbdff2d77b54302172afeab3c081a2ce88c`).
  **v3 Live filesystem greened** on existing-host WSL2 `/dev/kvm` at
  `2decde58…` (matrix report
...
- Supervised create ready-race no longer calls `wait_launcher` (which holds the
  ...
  performs the authentic `wait_launcher` once the launcher is gone. The same
  ready-race fix applies to supervised `exec` helpers.
...
- Retained clean-main x86_64 WSL2 Linux KVM Create durable reopen (11/11)
  evidence for revision `01786ab` against that same rebuilt system image.
  Observation-only; promotion gates remain open.
...
- Retained a current-source packaged-shim containerd qualification record.
  ...
  containerd 2.2.1 WSL2 matrices across all 23 restart, shim-rehydration, and
  forced-cleanup boundaries (95.09, 95.25, and 114.44 seconds). The
  compatibility record binds the source revision, package qualification
  report, executable and Cargo.lock digests, while keeping the result
  observation-only until a signed published archive and the remaining driver
